...
void ggml_cuda_op_get_rows(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Debug: inspect MoE weights (print once)
    static bool getrows_debug_printed = false;
    if (!getrows_debug_printed && dst->name[0] != '\0' && strstr(dst->name, "ffn_moe_weights") != nullptr) {
        getrows_debug_printed = true;
        const int64_t n = std::min<int64_t>(8, dst->ne[0]);
        std::vector<float> h_row(n);
        CUDA_CHECK(cudaMemcpyAsync(h_row.data(), dst->data, n * sizeof(float), cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaStreamSynchronize(stream));
        fprintf(stderr, "[GETROWS DEBUG] %s ne0=%ld ne1=%ld first:", dst->name, (long) dst->ne[0], (long) dst->ne[1]);
        for (int64_t i = 0; i < n; ++i) fprintf(stderr, " %.6f", h_row[i]);
        fprintf(stderr, "\n");
        for (int64_t i = 0; i < n; ++i) {
            if (!std::isfinite(h_row[i])) {
                fprintf(stderr, "[GETROWS DEBUG] non-finite detected\n");
                break;
            }
        }
    }

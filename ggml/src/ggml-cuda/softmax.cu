...
void ggml_cuda_op_soft_max(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Debug: check MoE probs for n_expert_used=2 (print once)
    static bool softmax_debug_printed = false;
    if (!softmax_debug_printed && dst->name[0] != '\0' && strstr(dst->name, "ffn_moe_probs") != nullptr) {
        softmax_debug_printed = true;
        std::vector<float> h_row(std::min<int64_t>(8, dst->ne[0]));
        CUDA_CHECK(cudaMemcpyAsync(h_row.data(), dst->data, h_row.size() * sizeof(float), cudaMemcpyDeviceToHost, stream));
        CUDA_CHECK(cudaStreamSynchronize(stream));
        fprintf(stderr, "[SOFTMAX DEBUG] %s ne0=%ld ne1=%ld first:", dst->name, (long) dst->ne[0], (long) dst->ne[1]);
        for (size_t i = 0; i < h_row.size(); ++i) fprintf(stderr, " %.6f", h_row[i]);
        fprintf(stderr, "\n");
        for (size_t i = 0; i < h_row.size(); ++i) {
            if (!std::isfinite(h_row[i])) {
                fprintf(stderr, "[SOFTMAX DEBUG] non-finite detected\n");
                break;
            }
        }
    }

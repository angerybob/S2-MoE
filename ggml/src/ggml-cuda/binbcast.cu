...
void ggml_cuda_op_add(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (getenv("GGML_MOE_NAN_DEBUG") && dst->name[0] != '\0' && strstr(dst->name, "ffn_moe_out") != nullptr) {
        ...
        if (src0 && src1 && src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32) {
            ...
            CUDA_CHECK(cudaMemcpyAsync(h0, src0->data, sizeof(h0), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaMemcpyAsync(h1, src1->data, sizeof(h1), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaStreamSynchronize(stream));
            fprintf(stderr, "[MOE ADD IN] %s src0:", dst->name);
            for (int i = 0; i < 8; ++i) fprintf(stderr, " %.6f", h0[i]);
            fprintf(stderr, "\n");
            fprintf(stderr, "[MOE ADD IN] %s src1:", dst->name);
            for (int i = 0; i < 8; ++i) fprintf(stderr, " %.6f", h1[i]);
            fprintf(stderr, "\n");
        }
    }
}
...
void ggml_cuda_op_mul(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (getenv("GGML_MOE_WEIGHT_DEBUG") && dst->name[0] != '\0' && strstr(dst->name, "ffn_moe_weighted") != nullptr) {
        const ggml_tensor * src0 = dst->src[0];
        const ggml_tensor * src1 = dst->src[1];
        cudaStream_t stream = ctx.stream();

        const int64_t n_check = std::min<int64_t>(256, ggml_nelements(dst));
        std::vector<float> h_dst(n_check);

        if (dst->type == GGML_TYPE_F32) {
            CUDA_CHECK(cudaMemcpyAsync(h_dst.data(), dst->data, n_check * sizeof(float), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaStreamSynchronize(stream));
        } else if (dst->type == GGML_TYPE_F16) {
            std::vector<ggml_fp16_t> tmp(n_check);
            CUDA_CHECK(cudaMemcpyAsync(tmp.data(), dst->data, n_check * sizeof(ggml_fp16_t), cudaMemcpyDeviceToHost, stream));
            CUDA_CHECK(cudaStreamSynchronize(stream));
            for (int64_t i = 0; i < n_check; ++i) {
                h_dst[i] = ggml_fp16_to_fp32(tmp[i]);
            }
        } else {
            return;
        }
        ...
        int inf_count = 0;
        for (int64_t i = 0; i < n_check; ++i) {
            if (!std::isfinite(h_dst[i])) {
                if (std::isnan(h_dst[i])) nan_count++;
                else inf_count++;
            }
        }
        ...
        if (nan_count > 0 || inf_count > 0) {
            fprintf(stderr,
                    "[MOE WEIGHTED DEBUG] %s dst type=%s ne=[%ld,%ld,%ld,%ld] nb=[%zu,%zu,%zu,%zu] nan=%d inf=%d\n",
            ...
            if (src0) {
                fprintf(stderr,
                        "[MOE WEIGHTED DEBUG] src0=%s type=%s ne=[%ld,%ld,%ld,%ld] nb=[%zu,%zu,%zu,%zu]\n",
            ...
            if (src1) {
                fprintf(stderr,
                        "[MOE WEIGHTED DEBUG] src1=%s type=%s ne=[%ld,%ld,%ld,%ld] nb=[%zu,%zu,%zu,%zu]\n",
            ...
            if (src0 && src0->type == GGML_TYPE_F32) {
                CUDA_CHECK(cudaMemcpyAsync(h0, src0->data, sizeof(h0), cudaMemcpyDeviceToHost, stream));
                CUDA_CHECK(cudaStreamSynchronize(stream));
                fprintf(stderr, "[MOE WEIGHTED DEBUG] src0 first8:");
                for (int i = 0; i < 8; ++i) fprintf(stderr, " %.6f", h0[i]);
                fprintf(stderr, "\n");
            }
            if (src1 && src1->type == GGML_TYPE_F32) {
                CUDA_CHECK(cudaMemcpyAsync(h1, src1->data, sizeof(h1), cudaMemcpyDeviceToHost, stream));
                CUDA_CHECK(cudaStreamSynchronize(stream));
                fprintf(stderr, "[MOE WEIGHTED DEBUG] src1 first8:");
                for (int i = 0; i < 8; ++i) fprintf(stderr, " %.6f", h1[i]);
                fprintf(stderr, "\n");
            }
        }

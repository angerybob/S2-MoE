    ...
    switch (n_expert) {
        case 1:
            topk_moe_cuda<1, with_norm>
                <<<grid_dims, block_dims, nbytes_shared, stream>>>(logits, weights, ids, n_rows, ids_stride, n_expert_used);
            break;
        case 2:
...
void ggml_cuda_op_topk_moe(ggml_backend_cuda_context & ctx,
    ...
    GGML_ASSERT(ids_stride >= n_expert_used);

    if (with_norm) {
        launch_topk_moe_cuda<true>(ctx, logits_d, weights_d, ids_d, n_rows, n_experts, n_expert_used, ids_stride);
    } else {
        launch_topk_moe_cuda<false>(ctx, logits_d, weights_d, ids_d, n_rows, n_experts, n_expert_used, ids_stride);
    }

    // Debug: check weights/ids for n_expert_used==2 (print once)
    static bool debug_printed = false;
    if (!debug_printed && n_expert_used == 2 && n_rows > 0) {
        debug_printed = true;
        std::vector<float> w_host(n_expert_used);
        std::vector<int32_t> ids_host(n_expert_used);
        CUDA_CHECK(cudaMemcpyAsync(w_host.data(), weights->data, n_expert_used * sizeof(float), cudaMemcpyDeviceToHost, ctx.stream()));
        CUDA_CHECK(cudaMemcpyAsync(ids_host.data(), ids->data, n_expert_used * sizeof(int32_t), cudaMemcpyDeviceToHost, ctx.stream()));
        CUDA_CHECK(cudaStreamSynchronize(ctx.stream()));
        fprintf(stderr, "[TOPK DEBUG] n_rows=%d n_expert_used=%d weights: %.6f %.6f ids: %d %d\n",
                n_rows, n_expert_used, w_host[0], w_host[1], ids_host[0], ids_host[1]);
        if (!std::isfinite(w_host[0]) || !std::isfinite(w_host[1])) {
            fprintf(stderr, "[TOPK DEBUG] weights non-finite\n");
        }
    }
}

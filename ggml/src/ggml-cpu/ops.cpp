...
static bool ggml_moe_debug_enabled() {
    static int cached = -1;
    if (cached == -1) {
        cached = getenv("GGML_MOE_DEBUG") ? 1 : 0;
    }
    return cached == 1;
}

static bool ggml_moe_debug_match(const ggml_tensor * t, const char * key) {
    return t && t->name && key && strstr(t->name, key) != nullptr;
}
...
void ggml_compute_forward_get_rows(
    ...
    if (params->ith == 0 && ggml_moe_debug_enabled() && ggml_moe_debug_match(dst, "ffn_moe_weights")) {
        ...
        if (dst->type == GGML_TYPE_F32) {
            const float * data = (const float *) dst->data;
            ...
            fprintf(stderr,
                    "[MOE CPU GET_ROWS] %s ne=[%ld,%ld,%ld,%ld] nan=%d inf=%d min=%f max=%f\n",
                    dst->name, (long) dst->ne[0], (long) dst->ne[1], (long) dst->ne[2], (long) dst->ne[3],
                    nan_count, inf_count, min_v, max_v);
            const int64_t n_print = std::min<int64_t>(8, n);
            fprintf(stderr, "[MOE CPU GET_ROWS] first:");
            for (int64_t i = 0; i < n_print; ++i) {
                fprintf(stderr, " %.6f", data[i]);
            }
            fprintf(stderr, "\n");
        } else if (dst->type == GGML_TYPE_F16) {
            const ggml_fp16_t * data = (const ggml_fp16_t *) dst->data;
            for (int64_t i = 0; i < n; ++i) {
            ...
            fprintf(stderr,
                    "[MOE CPU GET_ROWS] %s ne=[%ld,%ld,%ld,%ld] nan=%d inf=%d min=%f max=%f (f16)\n",
                    dst->name, (long) dst->ne[0], (long) dst->ne[1], (long) dst->ne[2], (long) dst->ne[3],
                    nan_count, inf_count, min_v, max_v);
        }
    }
...
void ggml_compute_forward_soft_max(
    ...
    if (ggml_moe_debug_enabled() && ggml_moe_debug_match(dst, "ffn_moe_")) {
        const float * data = (const float *) dst->data;
        const int64_t n = ggml_nelements(dst);
        int nan_count = 0;
        int inf_count = 0;
        ...
        fprintf(stderr,
                "[MOE CPU SOFTMAX] %s ne=[%ld,%ld,%ld,%ld] nan=%d inf=%d min=%f max=%f\n",
                dst->name, (long) dst->ne[0], (long) dst->ne[1], (long) dst->ne[2], (long) dst->ne[3],
                nan_count, inf_count, min_v, max_v);
        const int64_t n_print = std::min<int64_t>(8, n);
        fprintf(stderr, "[MOE CPU SOFTMAX] first:");
        for (int64_t i = 0; i < n_print; ++i) {
            fprintf(stderr, " %.6f", data[i]);
        }
        fprintf(stderr, "\n");
...
// ggml_compute_forward_moe_reuse_two_pass

void ggml_compute_forward_moe_reuse_two_pass(
    ...
    const int64_t ne0 = src0->ne[0]; // experts
    const int64_t ne1 = src0->ne[1]; // tokens

    const size_t nb0 = src0->nb[0];
    const size_t nb1 = src0->nb[1];

    GGML_ASSERT(nb0 == sizeof(float));
    GGML_ASSERT(nb1 == (size_t) ne0 * sizeof(float));
    GGML_ASSERT(dst->nb[0] == nb0 && dst->nb[1] == nb1);
    ...
    if (no_op) {
        memcpy(dst->data, src0->data, nb1 * ne1);
        return;
    }

    std::vector<float> w_raw(ne1, 0.0f);
    float max_w_raw = 0.0f;

    // compute token importance w_raw[b] = max(0, max - mean)
    for (int64_t b = 0; b < ne1; ++b) {
        const float * src_row = (const float *) ((const char *) src0->data + b * nb1);
        float sum = 0.0f;
        float mx  = -INFINITY;
        for (int64_t e = 0; e < ne0; ++e) {
            const float v = src_row[e];
            sum += v;
            mx = std::max(mx, v);
        }
        const float mean = sum / (float) ne0;
        const float w = std::max(0.0f, mx - mean);
        w_raw[b] = w;
        max_w_raw = std::max(max_w_raw, w);
    }
    ...
    const float w_scale = 1.0f / max_w_raw;

    std::vector<float> g(ne0, 0.0f);
    for (int64_t b = 0; b < ne1; ++b) {
        const float w = w_raw[b] * w_scale;
        if (w == 0.0f) {
            continue;
        }
        const float * src_row = (const float *) ((const char *) src0->data + b * nb1);
    ...
    int32_t k_global = std::min<int32_t>((int32_t) ne0, cap);
    if (k_global < top_k) {
        k_global = top_k;
    }

    std::vector<int32_t> indices(ne0);
    std::iota(indices.begin(), indices.end(), 0);
    std::partial_sort(indices.begin(), indices.begin() + k_global, indices.end(), [&](int a, int b) {
        if (g[a] == g[b]) {
            return a < b;
        }
        return g[a] > g[b];
    ...
    std::vector<float> bias(ne0, 0.0f);
    for (int32_t i = 0; i < k_global; ++i) {
        bias[indices[i]] = lambda;
    }

    for (int64_t b = 0; b < ne1; ++b) {
        const float * src_row = (const float *) ((const char *) src0->data + b * nb1);
        float *       dst_row = (float *)       ((char *) dst->data   + b * nb1);
        for (int64_t e = 0; e < ne0; ++e) {
            dst_row[e] = src_row[e] + bias[e];
        }
    }
}

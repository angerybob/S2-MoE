...
struct common_params_speculative {
    ...
    int32_t prune        =     0;
    int32_t prune_max_depth = 10;
    int32_t prune_max_nodes = 10;
    ...
    int32_t utility_test_iters = 4;  // samples per K candidate in test phase
    int32_t utility_set_iters  = 16; // set-phase length (iterations); doubled when best_k==0 (backoff)
};

// 在 common.h 中 struct common_params 定义之前添加：
struct common_params_speculative_pruning {
...
struct common_params {
    ...
    struct common_params_speculative_pruning sprune;
    ...
    bool answer_ppl      = false; // compute conditional PPL over answer tokens only
    ...
    std::string dataset_path      = ""; 
        // [新增] 限制问题数量，默认 -1 表示不限制
    int32_t n_questions_limit = -1; 
    bool use_ssd_moe       = false; // 默认为 false (全加载到内存)
    std::string hot_experts_path; // 存储 JSON 文件路径
    int32_t moe_expert_topk = -1; // MoE: main model experts per layer; <=0 = GGUF default
};

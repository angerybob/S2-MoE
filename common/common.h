...
struct common_params_speculative {
    ...
    int32_t n_expert_used =   -1; // override number of active experts for draft MoE models
    float   p_split      =  0.1f; // speculative decoding split probability
    float   p_min        = 0.75f; // minimum speculative decoding probability (greedy)
    int32_t prune        =     0; // enable marginal-benefit/cost draft-tree pruning
...
struct common_params_speculative_pruning {
    ...
    float tpot          = 64.2f;
    float eps           = 23.0f;
    float expert_bytes  = 22.528f;
    float bandwidth     = 204.8f;
    int   expert_max    = 384;
    float conf_thresh   = -0.97f;

...
static std::vector<int32_t> parse_int_list(const std::string & value) {
    ...
    for (const auto & part_raw : parts) {
        const std::string part = string_strip(part_raw);
        if (part.empty()) {
            continue;
        }
        try {
            ...
            throw std::invalid_argument(string_format("invalid integer: %s", part.c_str()));
        }
    }
    return out;
}
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    ).set_examples({LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_EMBEDDING, LLAMA_EXAMPLE_RETRIEVAL, LLAMA_EXAMPLE_PERPLEXITY, LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_BENCH}));
    add_opt(common_arg(
        {"--spm-infill"},
        string_format(
    ...
    add_opt(common_arg(
        {"--ssd-moe"},
        "enable SSD offloading for MoE experts (requires split GGUF)",
        [](common_params & params) {
            params.use_ssd_moe = true;
        }
    ).set_env("LLAMA_ARG_SSD_MOE"));
    // --- INSERT END ---
    add_opt(common_arg(
        {"--numa"}, "TYPE",
    ...
    add_opt(common_arg(
        {"--moe-reuse-strength"}, "N",
        "strength of MoE expert reuse bias (importance-aware router bias); 0 disables, negative uses model default",
        [](common_params & params, const std::string & value) {
            params.moe_reuse_strength = std::stof(value);
        }
    ).set_env("LLAMA_ARG_MOE_REUSE_STRENGTH"));
    add_opt(common_arg(
        {"--moe-reuse-expert-cap"}, "N",
        "cap the number of experts encouraged to activate per MoE call; 0 disables, negative uses model default",
        [](common_params & params, const std::string & value) {
            params.moe_reuse_expert_cap = std::stoi(value);
        }
    ...
    add_opt(common_arg(
        ...
        [](common_params & params) {
            params.speculative.share_kv = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE})); 
    // [新增] 注册 --dataset 参数
    add_opt(common_arg(
        {"--dataset"}, "PATH",
        "path to dataset file (e.g., datasets/gsm8K.jsonl)",
        [](common_params & params, const std::string & value) {
            params.dataset_path = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE,LLAMA_EXAMPLE_MAIN})); // 指定只在 speculative 示例中显示帮助
    // [新增] 注册 --n-questions 参数
    add_opt(common_arg(
        {"--n-questions"}, "N",
        "limit the number of questions to process (default: -1 = all)",
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params) {
            params.speculative.accept_log = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE})); 
    add_opt(common_arg(
        {"--cpu-moe", "-cmoe"},
    ...
    add_opt(common_arg(
        {"--answer-ppl"},
        string_format("compute conditional perplexity over answer tokens only (default: %s)", params.answer_ppl ? "true" : "false"),
        [](common_params & params) {
            params.answer_ppl = true;
        }
    ).set_examples({LLAMA_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"-pps"},
    ...
    add_opt(common_arg(
        {"--draft-skip-attn"}, "<layer,layer,...>",
        "comma-separated attention layer ids to skip in the draft model",
        [](common_params & params, const std::string & value) {
            params.speculative.draft_skip_attn_layers = parse_int_list(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--draft-skip-mlp"}, "<layer,layer,...>",
        "comma-separated MLP layer ids to skip in the draft model",
        [](common_params & params, const std::string & value) {
            params.speculative.draft_skip_mlp_layers = parse_int_list(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--draft-expert-topk"}, "N",
        "override draft MoE expert top-k for all layers (negative = model default)",
        [](common_params & params, int value) {
            params.speculative.draft_expert_topk = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--draft-layer-topk"}, "<k0,k1,...>",
        "comma-separated per-layer MoE expert top-k for the draft model",
        [](common_params & params, const std::string & value) {
            params.speculative.draft_layer_expert_topk = parse_int_list(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--search-stdin"},
        "speculative: read per-run configs from stdin (one JSON line per run)",
        [](common_params & params) {
            params.speculative.search_stdin = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--moe-log"}, "<0|1>",
        string_format("enable MoE accept/ratio logging for speculative decoding (default: %s)", params.moe_log ? "true" : "false"),
        [](common_params & params, const std::string & value) {
            params.moe_log = std::stoul(value) != 0;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--prune"}, "N",
        "enable speculative draft tree pruning (0 = off, 1 = on)",
        [](common_params & params, int value) {
            params.speculative.prune = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--prune-max-depth"}, "N",
        string_format("max draft tree depth for pruning (default: %d)", params.speculative.prune_max_depth),
        [](common_params & params, int value) {
            params.speculative.prune_max_depth = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--prune-max-nodes"}, "N",
        string_format("max draft tree nodes for pruning (default: %d)", params.speculative.prune_max_nodes),
        [](common_params & params, int value) {
            params.speculative.prune_max_nodes = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--eagle3"},
        "use native EAGLE3 speculative decoding with the draft model",
        [](common_params & params) {
    ...
    add_opt(common_arg(
        {"--moe-utility-spec"},
        "MoE: enable utility-driven adaptive draft length (test/set phases; see Cascade, arXiv:2506.20675)",
        [](common_params & params) {
            params.speculative.moe_utility_spec = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--spec-utility-test-iters"}, "N",
        string_format("utility-spec: draft samples per K during test phase (default: %d)", params.speculative.utility_test_iters),
        [](common_params & params, int value) {
            params.speculative.utility_test_iters = std::max(1, value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--spec-utility-set-iters"}, "N",
        string_format("utility-spec: decode iterations per set phase after choosing best K (default: %d)", params.speculative.utility_set_iters),
        [](common_params & params, int value) {
            params.speculative.utility_set_iters = std::max(1, value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    // =========================================================================
    // [新增] 自定义剪枝参数 (Pruning Params)
    // =========================================================================

    add_opt(common_arg(
        {"--prune-budget"}, "N",
        string_format("pruning: budget b (default: %.1f)", (double)params.sprune.budget_b),
        [](common_params & params, const std::string & value) {
            params.sprune.budget_b = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-m-route"}, "N",
        string_format("pruning: m route (default: %d)", params.sprune.m_route),
        [](common_params & params, int value) {
            params.sprune.m_route = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-k-tgt"}, "N",
        string_format("pruning: k tgt (default: %d)", params.sprune.k_tgt),
        [](common_params & params, int value) {
            params.sprune.k_tgt = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-beta"}, "N",
        string_format("pruning: beta (default: %.1f)", (double)params.sprune.beta),
        [](common_params & params, const std::string & value) {
            params.sprune.beta = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-gamma"}, "N",
        string_format("pruning: gamma (default: %.1f)", (double)params.sprune.gamma),
        [](common_params & params, const std::string & value) {
            params.sprune.gamma = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-lambda"}, "N",
        string_format("pruning: lambda (default: %.1f)", (double)params.sprune.lambda),
        [](common_params & params, const std::string & value) {
            params.sprune.lambda = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-tpot"}, "N",
        string_format("pruning: tpot (default: %.1f)", (double)params.sprune.tpot),
        [](common_params & params, const std::string & value) {
            params.sprune.tpot = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-eps"}, "N",
        string_format("pruning: eps (default: %.1f)", (double)params.sprune.eps),
        [](common_params & params, const std::string & value) {
            params.sprune.eps = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-expert-bytes"}, "N",
        string_format("pruning: expert bytes (default: %.3f)", (double)params.sprune.expert_bytes),
        [](common_params & params, const std::string & value) {
            params.sprune.expert_bytes = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-bandwidth"}, "N",
        string_format("pruning: bandwidth (default: %.1f)", (double)params.sprune.bandwidth),
        [](common_params & params, const std::string & value) {
            params.sprune.bandwidth = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));

    add_opt(common_arg(
        {"--prune-expert-max"}, "N",
        string_format("pruning: expert max (default: %d)", params.sprune.expert_max),
        [](common_params & params, int value) {
            params.sprune.expert_max = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            params.sprune.routing_noise = std::clamp(std::stof(value), 0.0f, 1.0f);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--prune-routing-noise-seed"}, "N",
        string_format("pruning: routing-noise random seed (default: %d)", params.sprune.routing_noise_seed),
        [](common_params & params, int value) {
            params.sprune.routing_noise_seed = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--prune-routing-noise-experts"}, "N",
        string_format("pruning: experts per layer used by routing-noise injection (default: %d)",
                      params.sprune.routing_noise_experts),
        [](common_params & params, int value) {
            params.sprune.routing_noise_experts = std::max(0, value);
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    // [新增] Trace 开关解析
    add_opt(common_arg(
        {"--prune-trace"},
        "enable detailed trace collection (logits, experts) - affects performance",
        [](common_params & params) {
            params.sprune.enable_trace = true;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"-cd", "--ctx-size-draft"}, "N",
    ...
    add_opt(common_arg(
        {"--hot-experts"}, "FILE",
        "path to JSON file containing hot experts list (format: {\"gpu_experts\": [[layer, expert], ...]})",
        [](common_params & params, const std::string & value) {
            params.hot_experts_path = value;
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_BENCH, LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--moe-expert-topk"}, "K",
        "main model MoE: number of experts to activate per layer (1..n_expert); <=0 = GGUF default (n_expert_used)",
        [](common_params & params, const std::string & value) {
            params.moe_expert_topk = std::stoi(value);
        }
    ).set_examples({LLAMA_EXAMPLE_MAIN, LLAMA_EXAMPLE_BENCH, LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(

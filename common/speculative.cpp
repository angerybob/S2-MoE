...
static llama_tokens gen_eagle3_draft(
    ...
    auto sample_and_check = [&](int idx) -> bool {
        const llama_token sampled_id = common_sampler_sample(smpl, ctx_decoder, idx);

        const auto * cur_p = common_sampler_get_candidates(smpl, false);
        GGML_ASSERT(cur_p->selected != -1 && "no selected token in draft sampler");
        const llama_token id = sampled_id;

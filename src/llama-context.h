...
struct llama_context {
    ...
    void set_draft_context(bool is_draft) { is_draft_context = is_draft; }
    bool is_draft_ctx() const { return is_draft_context; }

    // Draft-only execution controls (skip layers / MoE routing)
    void set_draft_skip_layers(const std::vector<int32_t> & attn_layers, const std::vector<int32_t> & mlp_layers);
    const std::vector<int32_t> & get_draft_skip_attn_layers() const;
    const std::vector<int32_t> & get_draft_skip_mlp_layers() const;
    void set_draft_expert_topk(int32_t topk);
    int32_t get_draft_expert_topk() const;
    void set_moe_topk_log_k(int32_t topk);
    int32_t get_moe_topk_log_k() const;
    void set_draft_layer_expert_topk(const std::vector<int32_t> & layer_topk);
    const std::vector<int32_t> & get_draft_layer_expert_topk() const;
    void clear_draft_layer_expert_topk();
    ...
    void extract_moe_topk_from_ubatch(const llm_graph_result * res, const llama_ubatch & ubatch, uint32_t n_tokens_all);
...
// 迁移到public部分
    ...
    std::vector<moe_topk_layer_data> moe_topk_layers_log;

...
bool llama_batch_allocr::init(
    ...
    if (batch.token) {
        for (int32_t i = 0; i < batch.n_tokens; ++i) {
            // EAGLE架构特殊处理：EAGLE模型可能接收>32k的token（来自target模型）
            // 但vocab.n_tokens()报告的是draft模型的vocab size (32k)
            // 我们通过检查token范围来识别可能的EAGLE输入模式
            bool is_potential_eagle_input = ((uint32_t) batch.token[i] >= 32000 &&
                                           (uint32_t) batch.token[i] <= 151943 &&
                                           vocab.n_tokens() == 32000);

            if (batch.token[i] < 0 ||
                (!is_potential_eagle_input && (uint32_t) batch.token[i] >= vocab.n_tokens())) {
                LLAMA_LOG_ERROR("%s: invalid token[%d] = %d, vocab_size=%d\n",
                               __func__, i, batch.token[i], vocab.n_tokens());
                return false;
            }
        }
    }

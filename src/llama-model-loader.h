...
struct llama_model_loader {
    ...
    // 存储热专家索引：Set<pair<layer_id, expert_id>>
    std::set<std::pair<int, int>> hot_expert_indices;
    
    // 解析 JSON 文件的辅助函数
    void load_hot_experts(const char* path);
    bool is_tensor_hot(const std::string & name) const;
// --- INSERT END ---
    ...
    llama_model_loader(
        ...
        bool use_mmap,
        bool check_tensors,
        bool use_ssd_moe, // <--- Add this argument

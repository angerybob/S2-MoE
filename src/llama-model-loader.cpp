...
static bool is_split_expert_tensor(const std::string & name) {
    // 目标格式: blk.L.ffn_xxx.NNN.weight
    // 1. 必须包含 ffn_ (gate/up/down)
    if (name.find("ffn_") == std::string::npos) return false;
    
    // 2. 必须以 .weight 结尾
    size_t pos = name.rfind(".weight");
    if (pos == std::string::npos || pos < 4) return false;
    
    // 3. 检查 .weight 前面是不是 .NNN (三位数字)
    // name[pos] 是 '.', name[pos-1] 是数字, ..., name[pos-4] 是 '.'
    if (name[pos-4] == '.' && 
        isdigit(name[pos-3]) && 
        isdigit(name[pos-2]) && 
        isdigit(name[pos-1])) {
...
llama_model_loader::llama_model_loader(
        ...
        const char * hot_experts_path // <--- 新增参数
        ) {
    int trace = 0;
    ...
    // --- INSERT ---
    this->use_ssd_moe = use_ssd_moe;
    // --------------
    // 加载热专家
    if (hot_experts_path) {
        load_hot_experts(hot_experts_path);
    }
}
...
bool llama_model_loader::load_all_data(
    ...
    for (struct ggml_tensor * cur = ggml_get_first_tensor(ctx); cur != NULL; cur = ggml_get_next_tensor(ctx, cur)) {
        ...
        // --- INSERT START: Skip loading data for split experts (SSD Offload) ---
        // --- Modified Step B: Hybrid Offloading ---
// --- Modified Step B: Dynamic Hot Experts from JSON ---
        // if (use_ssd_moe && is_split_expert_tensor(ggml_get_name(cur))) {
        //     std::string name = ggml_get_name(cur);
            
        //     // 解析 Layer ID 和 Expert ID
        //     // 格式示例: blk.0.ffn_down.000.weight
        //     int layer_id = -1;
        //     int expert_id = -1;
            
        //     // 1. 解析 Layer ID ("blk." 和 "." 之间)
        ...
        //     // 3. 查表判断
        //     bool is_hot = false;
        //     // 如果解析成功，且在 Set 中找到 {layer, expert}
        //     if (layer_id != -1 && expert_id != -1) {
        //         if (hot_expert_indices.count({layer_id, expert_id})) {
        //             is_hot = true;
        //         }
        //     } else {
        //         // 如果解析失败，为了安全起见，可以默认为冷，或者打印警告
        //         LLAMA_LOG_WARN("%s: failed to parse ID from %s\n", __func__, name.c_str());
        //     }

        //     // 4. 执行加载策略
        //     if (!is_hot) {
        //         printf("is cold!\n");
        //         // 冷专家：跳过加载 (data = nullptr)，后续走 SSD JIT
        //         // LLAMA_LOG_DEBUG("%s: [SSD] skipping expert: L%d-E%d (%s)\n", __func__, layer_id, expert_id, name.c_str());
        //         continue; 
        //     } else {
        //         printf("is hot!\n");
        //         // 热专家：正常执行下面的逻辑，加载到 RAM (Orin GPU 可用)
        //         // LLAMA_LOG_INFO("%s: [RAM] loading expert: L%d-E%d (%s)\n", __func__, layer_id, expert_id, name.c_str());
        //     }
        // }
        // ------------------------------------------
        // ------------------------------------------
        // if (is_split_expert_tensor(ggml_get_name(cur))) {
        //     // 我们跳过这些张量的内存分配和数据读取。
        //     // 它们的 data 指针将保持为 NULL。
        //     // 它们的元数据（offset, size, file_idx）仍然存在于 loader->weights_map 中，
        //     // 稍后在 llama-model.cpp 中我们会把这些元数据提取出来存到 SSD 注册表中。
        //     LLAMA_LOG_DEBUG("%s: skipping load for SSD expert: %s\n", __func__, ggml_get_name(cur));
        //     continue;
        // }
        // // --- INSERT END ---

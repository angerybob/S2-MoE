...
void llama_model::load_hparams(llama_model_loader & ml) {
        ...
        case LLM_ARCH_EAGLE3:
            {
                ...
                std::array<int, 4> extract_layers = {};
                if (!ml.get_key_or_arr(LLM_KV_EAGLE3_EXTRACT_LAYERS, extract_layers, 3, false)) {
                    throw std::runtime_error("EAGLE3 model requires extract_layers metadata");
                }
                std::copy_n(extract_layers.begin(), 3, hparams.eagle3_extract_layers.begin());
    ...
    if (arch == LLM_ARCH_EAGLE) {
        ...
        // Load d2t map (Draft-to-Target)
        {
            ...
            if (kid < 0) {
                throw std::runtime_error("required metadata array '" + key + "' not found");
            }
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        //     return nullptr;
        // };
        auto create_tensor_str = [&](const std::string & name, const std::initializer_list<int64_t> & ne, int layer_id) {
            // 1. 获取元数据
            ...
            if (weight) {
                ...
                // 3. 询问 Loader：我是热专家吗？
                if (ml.is_tensor_hot(name)) {
                    ggml_backend_buffer_type_t buft = nullptr;
                    if (layer_id >= 0 && layer_id < (int) pimpl->dev_layer.size()) {
                        auto * buft_list = pimpl->dev_layer.at(layer_id).buft_list;
                        buft = select_weight_buft(hparams, t_meta, GGML_OP_MUL_MAT_ID, *buft_list);
                        if (!buft) {
                            buft = buft_list->front().second;
                        }
                    }
                    ggml_context * hot_ctx = buft ? ctx_for_buft(buft) : pimpl->ctxs[0].get();
                    ggml_tensor* t = ml.create_tensor(hot_ctx, name, ne, 0);
                    
                    // 记录指针！(虽然现在 data 是空的，但 load_all_data 后会有数据)
                    // 我们在 load_tensors 结尾的 "Update Hot Expert Pointers" 逻辑会再次确认它
                    info.data_ptr = t->data; 
                    
                    // LLAMA_LOG_INFO("[RAM] Scheduled Hot Expert for loading: %s\n", name.c_str());
                } else {
                    // 是冷专家 -> 不创建 tensor，data_ptr 为空
            ...
            case LLM_ARCH_QWEN3MOE:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (layer.ffn_gate_exps == nullptr) {
                            ...
                            for (int x = 0; x < n_expert; ++x) {
                                char name_buf[256];
                                // Qwen 的命名通常是 ffn_gate (gate_proj)
                                snprintf(name_buf, sizeof(name_buf), "blk.%d.ffn_gate.%03d.weight", i, x);
                                
                                ggml_tensor* t_meta = ml.get_tensor_meta(name_buf);
                                if (t_meta) ghost_type = t_meta->type;
                                create_tensor_str(std::string(name_buf), {n_embd, n_ff_exp}, i);
                            }
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_gate_exps = ggml_new_tensor_3d(pimpl->ctxs[0].get(), ghost_type, n_embd, n_ff_exp, n_expert);
                                ggml_set_name(layer.ffn_gate_exps, tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i).str().c_str());
                                layer.ffn_gate_exps->data = (void*)1;
                                if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) layer.ffn_gate_exps->buffer = layer.ffn_gate_inp->buffer;
                            }
                        }
                        ...
                        if (layer.ffn_down_exps == nullptr) {
                            ...
                            for (int x = 0; x < n_expert; ++x) {
                                char name_buf[256];
                                snprintf(name_buf, sizeof(name_buf), "blk.%d.ffn_down.%03d.weight", i, x);
                                ggml_tensor* t_meta = ml.get_tensor_meta(name_buf);
                                if (t_meta) ghost_type = t_meta->type;
                                create_tensor_str(std::string(name_buf), {n_ff_exp, n_embd}, i);
                            }
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_down_exps = ggml_new_tensor_3d(pimpl->ctxs[0].get(), ghost_type, n_ff_exp, n_embd, n_expert);
                                ggml_set_name(layer.ffn_down_exps, tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i).str().c_str());
                                layer.ffn_down_exps->data = (void*)1;
                                if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) layer.ffn_down_exps->buffer = layer.ffn_gate_inp->buffer;
                            }
                        }
                        ...
                        if (layer.ffn_up_exps == nullptr) {
                            ...
                            for (int x = 0; x < n_expert; ++x) {
                                char name_buf[256];
                                snprintf(name_buf, sizeof(name_buf), "blk.%d.ffn_up.%03d.weight", i, x);
                                ggml_tensor* t_meta = ml.get_tensor_meta(name_buf);
                                if (t_meta) ghost_type = t_meta->type;
                                create_tensor_str(std::string(name_buf), {n_embd, n_ff_exp}, i);
                            }
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_up_exps = ggml_new_tensor_3d(pimpl->ctxs[0].get(), ghost_type, n_embd, n_ff_exp, n_expert);
                                ggml_set_name(layer.ffn_up_exps, tn(LLM_TENSOR_FFN_UP_EXPS, "weight", i).str().c_str());
                                layer.ffn_up_exps->data = (void*)1;
                                if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) layer.ffn_up_exps->buffer = layer.ffn_gate_inp->buffer;
                            }
                        }
                    }
                } break;
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (layer.ffn_gate_exps == nullptr) {
                            ...
                            for (int x = 0; x < n_expert; ++x) {
                                ...
                                ggml_tensor* t_meta = ml.get_tensor_meta(name_buf);
                                if (t_meta) ghost_type = t_meta->type;
                                create_tensor_str(std::string(name_buf), {n_embd, n_ff}, i);
                            }
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_gate_exps = ggml_new_tensor_3d(pimpl->ctxs[0].get(), ghost_type, n_embd, n_ff, n_expert);
                                ggml_set_name(layer.ffn_gate_exps, tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i).str().c_str());
                                layer.ffn_gate_exps->data = (void*)1;
                                if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) layer.ffn_gate_exps->buffer = layer.ffn_gate_inp->buffer;
                            }
                        ...
                        if (layer.ffn_down_exps == nullptr) {
                            ...
                            for (int x = 0; x < n_expert; ++x) {
                                ...
                                ggml_tensor* t_meta = ml.get_tensor_meta(name_buf);
                                if (t_meta) ghost_type = t_meta->type;
                                create_tensor_str(std::string(name_buf), {n_ff, n_embd}, i);
                            }
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_down_exps = ggml_new_tensor_3d(pimpl->ctxs[0].get(), ghost_type, n_ff, n_embd, n_expert);
                                ggml_set_name(layer.ffn_down_exps, tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i).str().c_str());
                                layer.ffn_down_exps->data = (void*)1;
                                if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) layer.ffn_down_exps->buffer = layer.ffn_gate_inp->buffer;
                            }
                        ...
                        if (layer.ffn_up_exps == nullptr) {
                            ...
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_up_exps = ggml_new_tensor_3d(pimpl->ctxs[0].get(), ghost_type, n_embd, n_ff, n_expert);
                                ggml_set_name(layer.ffn_up_exps, tn(LLM_TENSOR_FFN_UP_EXPS, "weight", i).str().c_str());
                                layer.ffn_up_exps->data = (void*)1;
                                if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) layer.ffn_up_exps->buffer = layer.ffn_gate_inp->buffer;
                            }
                        ...
                        if (i < (int) hparams.n_layer_dense_lead) {
                            ...
                            if (layer.ffn_gate_exps == nullptr) {
                                ...
                                if (!pimpl->ctxs.empty()) {
                                    ...
                                    ggml_set_name(layer.ffn_gate_exps, tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i).str().c_str());
                                    layer.ffn_gate_exps->data = (void*)1;
                                    // DeepSeek 的 gate_inp 可能叫 ffn_gate_inp 或其他，如果为空就不用赋 buffer
                                    if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) layer.ffn_gate_exps->buffer = layer.ffn_gate_inp->buffer;
                                }
                            }
            ...
            case LLM_ARCH_OPENAI_MOE:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        auto & layer = layers[i];
                        // --- INSERT START: Get correct GPU context for this layer ---
                        // 获取当前层首选的 Buffer Type (通常是 CUDA)
                        // pimpl->dev_layer 存储了每层的设备分配信息
                        auto * buft_list = pimpl->dev_layer.at(i).buft_list;
                        ggml_backend_buffer_type_t buft = buft_list->front().second; 
                        
                        // 获取对应的 ggml_context
                        // 注意：这里复用了 load_tensors 前面定义的 ctx_for_buft lambda
                        ...
                        // layer.ffn_down_exps = create_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i), {n_ff_exp,   n_embd, n_expert}, 0);
                        // layer.ffn_up_exps   = create_tensor(tn(LLM_TENSOR_FFN_UP_EXPS,   "weight", i), {  n_embd, n_ff_exp, n_expert}, 0);
// --- MODIFICATION START: GPT-OSS SSD Offload Logic ---
                        
                        // [1. Gate Experts]
                        // 尝试加载完整的大张量
                        layer.ffn_gate_exps = create_tensor(tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i), {n_embd, n_ff_exp, n_expert}, TENSOR_NOT_REQUIRED);
                        
                        // 如果是 nullptr，说明是 Split 模型
                        if (layer.ffn_gate_exps == nullptr) {
                            ggml_type ghost_type = GGML_TYPE_F16; // 默认类型，防止获取失败

                            for (int x = 0; x < n_expert; ++x) {
                                char name_buf[256];
                                snprintf(name_buf, sizeof(name_buf), "blk.%d.ffn_gate.%03d.weight", i, x);
                                
                                // 1. 获取真实类型 (这对 GPT-OSS 的 MXFP4 很重要！)
                                // 如果不获取真实类型，CUDA 后端可能会用错误的 Kernel 计算导致乱码
                                ggml_tensor* t_meta = ml.get_tensor_meta(name_buf);
                                if (t_meta) ghost_type = t_meta->type;

                                // 2. 注册 SSD 信息
                                // 这里调用你之前写的 create_tensor_str 或者 register_ssd_tensor 都可以
                                // 只要能把 offset 存进 ssd_experts 就行
                                create_tensor_str(std::string(name_buf), {n_embd, n_ff_exp}, i);
                                
                            }

                            // 3. 【关键补丁】创建 Ghost Tensor 防止 Segfault
                            if (!pimpl->ctxs.empty()) {
                                // 使用 ghost_type (MXFP4) 创建空壳张量
                                layer.ffn_gate_exps = ggml_new_tensor_3d(layer_ctx,  ghost_type, n_embd, n_ff_exp, n_expert);
                                // LLAMA_LOG_INFO("Ghost Tensor Type for Gate: %s\n", ggml_type_name(layer.ffn_gate_exps->type));
                                
                                // 设置名字 (Graph 构建依赖名字)
                                ggml_set_name(layer.ffn_gate_exps, tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i).str().c_str());
                                
                                // 显式标记数据为空
                                // layer.ffn_gate_exps->data = nullptr; 

                                // [核心修正] 设为 0x1 (非空)，欺骗 Allocator 跳过分配
                                // 如果设为 nullptr，Allocator 会分配 20GB 内存！
                                layer.ffn_gate_exps->data = (void*)1; 
                                // --- [新增关键修复] ---
                                // "偷" gate_inp 的 buffer 指针赋给 exps
                                // 这会让调度器确信这个张量位于 GPU 上
                                // if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) {
                                //     layer.ffn_gate_exps->buffer = layer.ffn_gate_inp->buffer;
                                // } else {
                                //     // 如果连 gate_inp 都没有，那这就真没救了，随便找个 GPU buffer
                                //     // (通常不会发生，除非模型坏了)
                                //     LLAMA_LOG_WARN("Warning: No buffer found for ffn_gate_inp, Ghost Tensor might fail on scheduler\n");
                                // }
                                // ---------------------
                            }
                        }
                        ...
                        if (layer.ffn_down_exps == nullptr) {
                            ...
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_down_exps = ggml_new_tensor_3d(layer_ctx,  ghost_type, n_ff_exp, n_embd, n_expert);
                                ggml_set_name(layer.ffn_down_exps, tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i).str().c_str());
                                // layer.ffn_down_exps->data = nullptr;
                                layer.ffn_down_exps->data = (void*)1;
                                // --- [新增] ---
                                // 同样偷 gate_inp 的 buffer (它们肯定在同一张卡上)
                                // if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) {
                                //     layer.ffn_down_exps->buffer = layer.ffn_gate_inp->buffer;
                                // } else {
                                //     // 如果连 gate_inp 都没有，那这就真没救了，随便找个 GPU buffer
                                //     // (通常不会发生，除非模型坏了)
                                //     LLAMA_LOG_WARN("Warning: No buffer found for ffn_gate_inp, Ghost Tensor might fail on scheduler\n");
                        ...
                        if (layer.ffn_up_exps == nullptr) {
                            ...
                            if (!pimpl->ctxs.empty()) {
                                layer.ffn_up_exps = ggml_new_tensor_3d(layer_ctx,  ghost_type, n_embd, n_ff_exp, n_expert);
                                ggml_set_name(layer.ffn_up_exps, tn(LLM_TENSOR_FFN_UP_EXPS, "weight", i).str().c_str());
                                // layer.ffn_up_exps->data = nullptr;
                                layer.ffn_up_exps->data = (void*)1;
                                // --- [新增] ---
                                // if (layer.ffn_gate_inp && layer.ffn_gate_inp->buffer) {
                                //     layer.ffn_up_exps->buffer = layer.ffn_gate_inp->buffer;
                                // }
                            }
                        }
                        // --- MODIFICATION END ---
            ...
            case LLM_ARCH_EAGLE:
                {
                    ...
                    // EAGLE model tensors - 注意：tok_embd被标记为TENSOR_NOT_REQUIRED，因为EAGLE使用shared embedding
                    ea_layer.tok_embd = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_draft_vocab}, TENSOR_NOT_REQUIRED);
            ...
            case LLM_ARCH_EAGLE3:
                {
                    ...
                    if (!d2t_meta) {
                        throw std::runtime_error("EAGLE3 model requires d2t tensor");
                    }
                    ...
                    const std::string tok_embd_name = tn(LLM_TENSOR_TOKEN_EMBD, "weight").str();
                    const ggml_tensor * tok_embd_meta = ml.get_tensor_meta(tok_embd_name.c_str());
                    if (tok_embd_meta) {
                        tok_embd = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, tok_embd_meta->ne[1]}, 0);
                    }
...
// --- INSERT START: Fix Ghost Tensor Buffers ---
    // 此时所有的普通张量都已经分配了 buffer。
    // 但是我们的 Ghost Tensor (data == 0x1) 被 Allocator 跳过了，buffer 是 nullptr。
    // 我们需要遍历所有 Context，找到 Ghost Tensor，并随便指派一个同 Context 下的有效 buffer 给它。
    
    for (auto & ctx : pimpl->ctxs) {
        // 1. 先在这个 Context 里找一个有 Buffer 的“良民”
        ggml_backend_buffer_t valid_buffer = nullptr;
        for (ggml_tensor * t = ggml_get_first_tensor(ctx.get()); t != nullptr; t = ggml_get_next_tensor(ctx.get(), t)) {
            if (t->buffer != nullptr) {
                valid_buffer = t->buffer;
                break;
            }
        }

        // 如果这个 Context 里全是孤魂野鬼（不太可能），那就跳过
        if (valid_buffer == nullptr) continue;

        // 2. 再次遍历，给所有 Ghost Tensor 发身份证
        for (ggml_tensor * t = ggml_get_first_tensor(ctx.get()); t != nullptr; t = ggml_get_next_tensor(ctx.get(), t)) {
            // 判定标准：data是0x1 且 buffer为空
            if (t->data == (void*)1 && t->buffer == nullptr) {
                t->buffer = valid_buffer; // 强行绑定！
                
                // 顺便把 backend 标志也设一下（视版本而定，buffer通常足够了）
                // 这样调度器看到 t->buffer 指向 GPU buffer，就会把它调度到 GPU
                
                // LLAMA_LOG_INFO("Fixed buffer for Ghost Tensor: %s\n", t->name);
            }
        }
    }
    // --- INSERT END ---
...
// --- INSERT START: Update Hot Expert Pointers (Fixed) ---
    // 数据加载完毕后，我们需要从 Model 的 tensors_by_name 中找到那些“热专家”的真实数据指针
    if (!pimpl->ssd_experts.empty()) {
        int updated_count = 0;
        
        // 1. 构建一个临时的查找表：Name -> Tensor*
        // tensors_by_name 是 llama_model 的成员，包含了所有已创建的 tensor
        std::map<std::string, ggml_tensor*> loaded_tensors_map;
        for (const auto& item : tensors_by_name) {
            loaded_tensors_map[item.first] = item.second;
        }

        // 2. 遍历注册表，尝试更新指针
        for (auto & [name, info] : pimpl->ssd_experts) {
            // 在已加载的张量里找
            auto it = loaded_tensors_map.find(name);
            if (it != loaded_tensors_map.end()) {
                ggml_tensor* t = it->second;
                
                // 3. 检查是否有数据
                // 如果 Loader 判定它是“热”的，load_all_data 就会给它分配内存并读入数据
                // (void*)1 是我们要避开的 Ghost Tensor 标记
                if (t->data != nullptr && t->data != (void*)1) {
                    info.data_ptr = t->data;
                    updated_count++;
                }
            }
...
struct llm_build_qwen3moe : public llm_graph_context {
    llm_build_qwen3moe(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        // =================================================================
        // Draft-only skip layers / MoE routing controls (external config)
        const bool is_draft_model = is_draft_context;
        std::vector<char> skip_attn(n_layer, false);
        std::vector<char> skip_mlp(n_layer, false);
        if (is_draft_model) {
            for (const int32_t layer_id : draft_skip_attn_layers) {
                if (layer_id >= 0 && layer_id < n_layer) {
                    skip_attn[layer_id] = true;
                }
            }
            for (const int32_t layer_id : draft_skip_mlp_layers) {
                if (layer_id >= 0 && layer_id < n_layer) {
                    skip_mlp[layer_id] = true;
                }
            }
        }

        std::vector<int32_t> layer_topk_map;
        if (is_draft_model && !draft_layer_expert_topk.empty()) {
            layer_topk_map.assign(n_layer, -1);
            for (size_t i = 0; i < draft_layer_expert_topk.size() && i < (size_t) n_layer; ++i) {
                layer_topk_map[i] = draft_layer_expert_topk[i];
            }
        }
        // =================================================================
        GGML_ASSERT(n_embd_head == hparams.n_embd_head_k);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ggml_tensor * inpSA = inpL;
            capture_eagle3(inpL, il);
            ggml_tensor * attn_out = nullptr; // 用于存储 Attention 的输出（不含残差）
            ...
            // 检查是否跳过 Attention
            bool skip_this_attn = is_draft_model && skip_attn[il];
            ...
            if (!skip_this_attn) {
                ...
                attn_out = cur; // 保存 Attn 计算结果
            } else {
                // 如果跳过，attn_out 保持为 nullptr 或者不做处理
                // 实际在下方加法时处理
            }
            if (il == n_layer - 1 && inp_out_ids) {
                // cur   = ggml_get_rows(ctx0,   cur, inp_out_ids);
            ...
            // ggml_tensor * ffn_inp = ggml_add(ctx0, cur, inpSA);
            // Residual Connection: ffn_inp = attn_out + inpSA
            ggml_tensor * ffn_inp;
            if (attn_out) {
                ffn_inp = ggml_add(ctx0, attn_out, inpSA);
            } else {
                // 如果跳过 Attn，直接直通：ffn_inp = inpSA
                ffn_inp = inpSA; 
            }
            cb(ffn_inp, "ffn_inp", il);
            ...
            // MoE branch
            // 检查是否跳过 MLP
            bool skip_this_mlp = is_draft_model && skip_mlp[il];
            
            ggml_tensor * mlp_out_tensor = nullptr;
            if (!skip_this_mlp) {
                cur = build_norm(ffn_inp,
                        model.layers[il].ffn_norm, NULL,
                        LLM_NORM_RMS, il);
                ...
                // 确定当前层使用的 Top-K
                int current_k = n_expert_used;
                if (is_draft_model) {
                    if (!layer_topk_map.empty() && layer_topk_map[il] > 0) {
                        current_k = layer_topk_map[il];
                    } else if (draft_expert_topk > 0) {
                        current_k = draft_expert_topk;
                    }
                }

                // 确保不要越界 (Safe guard)
                if (current_k > n_expert_used) current_k = n_expert_used;
                if (current_k > n_expert) current_k = n_expert;
                ...
                ggml_tensor * moe_out =
                    build_moe_ffn(cur,
                            ...
                            model.layers[il].ffn_down_exps,
                            nullptr,
                            n_expert, current_k, // <--- 使用动态 Top-K
                            LLM_FFN_SILU, true,
                            false, 0.0,
                            LLAMA_EXPERT_GATING_FUNC_TYPE_SOFTMAX,
                            il);
                cb(moe_out, "ffn_moe_out", il);
                mlp_out_tensor = moe_out;
            }
            // cur = moe_out;

            // cur = ggml_add(ctx0, cur, ffn_inp);
            // Residual Connection: cur = mlp_out + ffn_inp
            if (mlp_out_tensor) {
                cur = ggml_add(ctx0, mlp_out_tensor, ffn_inp);
            } else {
                // 如果跳过 MLP，直接直通
                cur = ffn_inp;
            }
...
struct llm_build_deepseek2 : public llm_graph_context {
    llm_build_deepseek2(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        ggml_tensor * inp_out_ids = build_inp_out_ids();
        int64_t actual_n_layers = n_layer;
...
struct llm_build_eagle3 : public llm_graph_context {
    llm_build_eagle3(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        if (shared_token_embd) {
            LLAMA_LOG_INFO("EAGLE3: Using shared token embedding from target model\n");
            ...
            if (res->t_tokens) {
                ...
                LLAMA_LOG_INFO("EAGLE3: DEBUG - Added inp_tokens callback for token ID debugging\n");
            }
        } else {
            LLAMA_LOG_INFO("EAGLE3: Creating placeholder embedding input - will be replaced when shared_token_embd is set\n");
        ...
        LLAMA_LOG_INFO("EAGLE3: target_hs dims [%ld, %ld, %ld, %ld]\n",
                target_hs->ne[0], target_hs->ne[1], target_hs->ne[2], target_hs->ne[3]);
        LLAMA_LOG_INFO("EAGLE3: draft_hs dims [%ld, %ld, %ld, %ld]\n",
                draft_hs->ne[0], draft_hs->ne[1], draft_hs->ne[2], draft_hs->ne[3]);
        LLAMA_LOG_INFO("EAGLE3: ea_layer.fc dims [%ld, %ld, %ld, %ld]\n",
        ...
        ggml_tensor * target_hs_proj = ggml_mul_mat(ctx0, model.ea_layer.fc, target_hs);
        ...
        // EAGLE3 has only one layer, so no loop needed
        {
            ...
            {
                // compute Q and K and RoPE them
                LLAMA_LOG_INFO("EAGLE3: About to compute Qcur...\n");
                LLAMA_LOG_INFO("EAGLE3: wq dims [%ld, %ld], cur dims [%ld, %ld]\n",
                        model.ea_layer.wq->ne[0], model.ea_layer.wq->ne[1],
                        cur->ne[0], cur->ne[1]);
                ggml_tensor * Qcur = build_lora_mm(model.ea_layer.wq, cur);
                cb(Qcur, "Qcur", 0);

                LLAMA_LOG_INFO("EAGLE3: About to compute Kcur...\n");
                ggml_tensor * Kcur = build_lora_mm(model.ea_layer.wk, cur);
                cb(Kcur, "Kcur", 0);

                LLAMA_LOG_INFO("EAGLE3: About to compute Vcur...\n");
            ...
            LLAMA_LOG_INFO("EAGLE3: Before ffn_up - cur dims [%ld, %ld], ffn_up dims [%ld, %ld]\n",
                    cur->ne[0], cur->ne[1],
                    model.ea_layer.ffn_up->ne[0], model.ea_layer.ffn_up->ne[1]);
            LLAMA_LOG_INFO("EAGLE3: ffn_gate dims [%ld, %ld], ffn_down dims [%ld, %ld]\n",
...
std::map<std::string, ssd_expert_info_t> llama_model::get_ssd_experts() const {
    ...
    for (const auto& kv : pimpl->ssd_experts) {
        ...
        info.data_ptr = kv.second.data_ptr; 

...
// --- INSERT START ---
// 定义一个结构体来存储 SSD 专家的元数据
struct ssd_expert_info {
    uint16_t file_idx; // 位于哪个分割文件 (split file)
    size_t   file_offs; // 文件中的偏移量
    size_t   size;      // 数据大小 (bytes)
    ggml_type type;     // 数据类型 (MXFP4 / F16 等)
    std::vector<int64_t> ne; // 维度信息
};

// 注册表：Key 可以是 tensor name，或者 layer_id + expert_id + type 的组合
// 这里简单起见，用 tensor name 做 key
std::map<std::string, ssd_expert_info> ssd_experts;
// --- INSERT END ---
...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    ml.get_key(LLM_KV_MOE_REUSE_STRENGTH, hparams.moe_reuse_strength, false);
    ml.get_key(LLM_KV_DFT_EXIT_LAYER, hparams.dft_exit_layer, false);
    {
        uint32_t cap_tmp = 0;
        if (ml.get_key(LLM_KV_MOE_REUSE_EXPERT_CAP, cap_tmp, false)) {
            hparams.moe_reuse_expert_cap = (int32_t) cap_tmp;
        }
    }
    // {
    //     uint32_t layer_tmp = 0;
    //     if (ml.get_key(LLM_KV_DFT_EXIT_LAYER, layer_tmp, false)) {
    //         hparams.dft_exit_layer = (int32_t) layer_tmp;
    //     }
    // }
    if (params.moe_reuse_strength >= 0.0f) {
        hparams.moe_reuse_strength = params.moe_reuse_strength;
    }
    if (params.moe_reuse_expert_cap >= 0) {
        hparams.moe_reuse_expert_cap = params.moe_reuse_expert_cap;
    }
    if (params.n_expert_used > 0) {
        hparams.n_expert_used = params.n_expert_used;
    }
    if (params.dft_exit_layer >= 0) {
        hparams.dft_exit_layer = params.dft_exit_layer;
    }
    ...
    switch (arch) {
        ...
        case LLM_ARCH_EAGLE:
            {
                // EAGLE model uses RMS Normalization
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                ...
                if (ml.get_key_or_arr(LLM_KV_EAGLE3_EXTRACT_LAYERS, extract_layers_tmp, 3, false)) {
                    ...
                    // Legacy GPT-OSS EAGLE3 conversions used the default last-layer
                    // input index instead of the configured layer output index.
                    if (hparams.n_layer == 1 && hparams.n_embd == 2880 &&
                            hparams.eagle3_extract_layers == std::array<int32_t, 3>{2, 18, 33}) {
                        hparams.eagle3_extract_layers[2] = 34;
                    }
                    ...
                    hparams.eagle3_extract_layers = {2, (int) hparams.n_layer / 2, (int) hparams.n_layer - 3};
                }
                ...
                ml.get_key(LLM_KV_EAGLE3_NORM_BEFORE_FC, hparams.eagle3_norm_before_fc, false);

                // EAGLE is typically a single-layer Transformer model for speculative decoding
                if (hparams.n_layer == 1) {
                    // Standard EAGLE model configuration
                    type = LLM_TYPE_1B;
                } else {
                    // Non-standard EAGLE variant found
                    LLAMA_LOG_WARN("%s: EAGLE model has unexpected layer count of %d. Expected 1.\n",
    ...
    // Load EAGLE vocabulary mappings if this is an EAGLE model
    if (arch == LLM_ARCH_EAGLE) {
        LLAMA_LOG_INFO("%s: loading EAGLE vocabulary mappings...\n", __func__);

        gguf_context * ctx = ml.meta.get();

        // Load d2t map (Draft-to-Target)
        {
            const std::string key = "eagle.d2t_map";
            const int kid = gguf_find_key(ctx, key.c_str());
            if (kid < 0) {
                const ggml_tensor * d2t_meta = ml.get_tensor_meta("d2t");
                if (d2t_meta == nullptr) {
                    throw std::runtime_error("required metadata array '" + key + "' or tensor 'd2t' not found");
                }
                ea_layer.d2t_map.resize(d2t_meta->ne[0]);
            } else {
                const enum gguf_type type = gguf_get_arr_type(ctx, kid);
                if (type != GGUF_TYPE_INT32) {
                    throw std::runtime_error("metadata array '" + key + "' has wrong type, expected INT32");
                }

                const int n = gguf_get_arr_n(ctx, kid);
                ea_layer.d2t_map.resize(n);

                const void * data_ptr = gguf_get_arr_data(ctx, kid);
                memcpy(ea_layer.d2t_map.data(), data_ptr, n * sizeof(int32_t));
            }
        }

        // Load t2d map (Target-to-Draft)
        {
            const std::string key = "eagle.t2d_map";
            const int kid = gguf_find_key(ctx, key.c_str());
            if (kid < 0) {
                ea_layer.t2d_map.clear();
            } else {
                const enum gguf_type type = gguf_get_arr_type(ctx, kid);
                if (type != GGUF_TYPE_BOOL) {
                    throw std::runtime_error("metadata array '" + key + "' has wrong type, expected BOOL");
                }

                const int n = gguf_get_arr_n(ctx, kid);
                ea_layer.t2d_map.resize(n);

                const void * data_ptr = gguf_get_arr_data(ctx, kid);
                memcpy(ea_layer.t2d_map.data(), data_ptr, n * sizeof(uint8_t));
            }
        }
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor_str = [&](const std::string & full_name, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            // 这里只保留 create_tensor 里最核心的几行
            ggml_tensor * t_meta = ml.get_tensor_meta(full_name.c_str());
            if (!t_meta) {
                if (flags & TENSOR_NOT_REQUIRED) return nullptr;
                throw std::runtime_error("missing tensor " + full_name);
            }
            
            // 省略了 buffer type 选择逻辑，这里直接假设用 GPU buffer (pimpl->dev_layer[i].buft_list)
            // 或者简单点，直接调用 ml.create_tensor，但是需要 context
            
            // 为了省事，我们这里只做“注册”，不真正分配 buffer，因为这些是 SSD tensor
            // 这正是 Step C 的逻辑！
            
            // 直接复用 Step C 的核心注册逻辑：
            const auto * weight = ml.get_weight(full_name.c_str());
            if (weight) {
                ssd_expert_info info;
                info.file_idx  = weight->idx;
                info.file_offs = weight->offs;
                info.size      = ggml_nbytes(t_meta);
                info.type      = t_meta->type;
                for(int i=0; i<GGML_MAX_DIMS; ++i) info.ne.push_back(t_meta->ne[i]);
                
                pimpl->ssd_experts[full_name] = info;
                
                // 返回一个 dummy tensor
                // 我们需要 ctx，这里可以用 ctx_map 里的第一个，或者新建一个临时的
                // 只要不分配 data 就行
                return nullptr; // 返回 nullptr 也没事，因为我们在外面本来就要赋给 nullptr
            }
            return nullptr;
        };
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            if (!t_meta) {
                ...
                // EAGLE3特殊处理：对于缺失的token_embd.weight，不要报错
                // 因为EAGLE3使用shared embedding，没有自己的token_embd
                if (arch == LLM_ARCH_EAGLE && tn.tensor == LLM_TENSOR_TOKEN_EMBD) {
                    LLAMA_LOG_INFO("%s: EAGLE3 model missing token_embd.weight - this is expected for shared embedding\n", __func__);
                    return nullptr;
                }
            ...
            // check overrides
	            ...
	            const bool is_split_routed_expert =
	                    std::regex_match(tensor_name, std::regex("^blk\\.[0-9]+\\.ffn_(gate|down|up)\\.[0-9]+\\.weight$"));

	            if (!buft && arch == LLM_ARCH_DEEPSEEK2 && !devices.empty() && !is_split_routed_expert &&
	                ...
	                buft = select_weight_buft(hparams, t_meta, op, pimpl->gpu_buft_list.at(dev));
	                if (buft) {
	                    LLAMA_LOG_DEBUG("tensor %s (%zu MiB %s) forced to GPU fixed-weight buffer type %s\n",
	                            tn.str().c_str(),
	                            ggml_nbytes(t_meta) / 1024 / 1024, ggml_type_name(t_meta->type),
	                            ggml_backend_buft_name(buft));
	                }
            ...
            // --- 修改为以下内容 (Step C: 拦截并注册 SSD 专家) ---
            
            // 1. 创建 ggml_tensor 结构体（分配图节点，但不分配 data 内存）
            ggml_tensor * tensor = ml.create_tensor(ctx, tn, ne, flags);

            if (tensor != nullptr) {
                std::string name = ggml_get_name(tensor);

                // 2. 检测是否是我们在 Step A 中拆分的专家权重
                // 特征：包含 "ffn_" (gate/up/down)，且以 ".weight" 结尾，且 ".weight" 前面是数字
                // 例如: blk.0.ffn_gate.000.weight
                bool is_ssd_expert = false;
                if (name.find("ffn_") != std::string::npos && name.find(".weight") != std::string::npos) {
                    size_t weight_pos = name.rfind(".weight");
                    if (weight_pos > 0 && isdigit(name[weight_pos - 1])) {
                        is_ssd_expert = true;
                    }
                }
                ...
                if (is_ssd_expert) {
                    ...
                    if (weight) {
                        // 4. 填充元数据到注册表 (pimpl->ssd_experts)
                        // 注意：这里需要你确保之前已经在 impl 里定义了 ssd_expert_info 结构体
                        ssd_expert_info info;
                        info.file_idx  = weight->idx;
                        info.file_offs = weight->offs;
                        ...
                        // 仅在调试时打印，防止刷屏
                        // LLAMA_LOG_DEBUG("%s: Registered SSD Expert: %s (Offset: %zu)\n", __func__, name.c_str(), weight->offs);
                    }
                }
            }
        ...
        ggml_context * split_moe_ghost_ctx = nullptr;
        auto ctx_for_split_moe_ghost = [&]() -> ggml_context * {
            if (split_moe_ghost_ctx == nullptr) {
                ggml_init_params params = {
                    /*.mem_size   =*/ ctx_size,
                    /*.mem_buffer =*/ NULL,
                    /*.no_alloc   =*/ true,
                ...
                split_moe_ghost_ctx = ggml_init(params);
                if (split_moe_ghost_ctx == nullptr) {
                    throw std::runtime_error(format("failed to create split MoE ghost ggml context"));
                }
                pimpl->ctxs.emplace_back(split_moe_ghost_ctx);
            }
            return split_moe_ghost_ctx;
        };

        auto create_split_moe_experts_tensor = [&](llm_tensor tensor_type, const char * split_name, int il,
                const std::initializer_list<int64_t> & merged_ne,
                const std::initializer_list<int64_t> & split_ne) -> ggml_tensor * {
            ggml_tensor * tensor = create_tensor(tn(tensor_type, "weight", il), merged_ne, TENSOR_NOT_REQUIRED);
            if (tensor != nullptr) {
                return tensor;
            }

            ggml_type ghost_type = GGML_TYPE_F16;
            for (int ie = 0; ie < n_expert; ++ie) {
                char name_buf[256];
                snprintf(name_buf, sizeof(name_buf), "blk.%d.%s.%03d.weight", il, split_name, ie);

                ggml_tensor * t_meta = ml.get_tensor_meta(name_buf);
                if (t_meta == nullptr) {
                    throw std::runtime_error(format("missing split expert tensor '%s'", name_buf));
                }
                ghost_type = t_meta->type;
                create_tensor_str(std::string(name_buf), split_ne, 0);
            }

            std::vector<int64_t> dims(merged_ne);
            GGML_ASSERT(dims.size() == 3);

            tensor = ggml_new_tensor_3d(ctx_for_split_moe_ghost(), ghost_type, dims[0], dims[1], dims[2]);
            ggml_set_name(tensor, tn(tensor_type, "weight", il).str().c_str());
            tensor->data = nullptr;
            return tensor;
        };
        ...
        switch (arch) {
            ...
            case LLM_ARCH_LLADA_MOE:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.ffn_gate_exps = create_split_moe_experts_tensor(
                                LLM_TENSOR_FFN_GATE_EXPS, "ffn_gate", i,
                                {  n_embd, n_ff_exp, n_expert}, {  n_embd, n_ff_exp});
                        layer.ffn_down_exps = create_split_moe_experts_tensor(
                                LLM_TENSOR_FFN_DOWN_EXPS, "ffn_down", i,
                                {n_ff_exp,   n_embd, n_expert}, {n_ff_exp,   n_embd});
                        layer.ffn_up_exps   = create_split_moe_experts_tensor(
                                LLM_TENSOR_FFN_UP_EXPS, "ffn_up", i,
                                {  n_embd, n_ff_exp, n_expert}, {  n_embd, n_ff_exp});
                    }
                } break;
            case LLM_ARCH_LLAMA4:
            ...
            case LLM_ARCH_OLMOE:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // MoE branch. Accept both merged expert tensors and split per-expert tensors.
                        layer.ffn_gate_exps = create_split_moe_experts_tensor(
                                LLM_TENSOR_FFN_GATE_EXPS, "ffn_gate", i,
                                {n_embd, n_ff,   n_expert}, {n_embd, n_ff});
                        layer.ffn_down_exps = create_split_moe_experts_tensor(
                                LLM_TENSOR_FFN_DOWN_EXPS, "ffn_down", i,
                                {n_ff,   n_embd, n_expert}, {n_ff,   n_embd});
                        layer.ffn_up_exps   = create_split_moe_experts_tensor(
                                LLM_TENSOR_FFN_UP_EXPS, "ffn_up", i,
                                {n_embd, n_ff,   n_expert}, {n_embd, n_ff});
                    }
            ...
            case LLM_ARCH_EAGLE:
                {
                    // EAGLE3特殊处理：对于output.weight，使用draft_vocab_size（32K）而不是full vocab size（151K）
                    // 因为EAGLE3只输出32K logits，通过d2t_map映射到target vocab space
                    int64_t n_draft_vocab = ea_layer.d2t_map.size();
                    LLAMA_LOG_INFO("%s: EAGLE3 model using draft vocab size %ld for output tensor\n", __func__, n_draft_vocab);

                    // Some EAGLE3 models carry a full target-vocab embedding, while older local drafts
                    // rely on the target model's shared embedding.
                    ea_layer.tok_embd = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab}, TENSOR_NOT_REQUIRED);

                    ea_layer.output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, 0);
                    ea_layer.output = create_tensor(tn(LLM_TENSOR_OUTPUT, "weight"), {n_embd, n_draft_vocab}, 0);
                    // Full-vocabulary EAGLE3 drafts can store an identity d2t map
                    // directly in metadata and do not need a redundant tensor.
                    ea_layer.d2t = create_tensor(tn(LLM_TENSOR_EAGLE_D2T), {n_draft_vocab}, TENSOR_NOT_REQUIRED);

                    // EAGLE fully connected layer
                    ea_layer.fc = create_tensor(tn(LLM_TENSOR_FC, "weight"), {n_embd*3, n_embd}, 0);
                    ea_layer.fc_b = create_tensor(tn(LLM_TENSOR_FC, "bias"), {n_embd}, TENSOR_NOT_REQUIRED);
                    ea_layer.input_norm = hparams.eagle3_norm_before_fc
                        ? create_tensor(tn(LLM_TENSOR_EAGLE_INPUT_NORM, "weight"), {n_embd*3}, TENSOR_NOT_REQUIRED)
                        : nullptr;

                    for (int i = 0; i < n_layer; ++i) {
                        ea_layer.wq = create_tensor(tn(LLM_TENSOR_ATTN_Q,   "weight", i), {n_embd*2, n_embd_head_k * n_head}, 0);
                        ea_layer.wk = create_tensor(tn(LLM_TENSOR_ATTN_K,   "weight", i), {n_embd*2, n_embd_k_gqa}, 0);
                        ea_layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V,   "weight", i), {n_embd*2, n_embd_v_gqa}, 0);
                        ea_layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i), {n_embd_head_k * n_head, n_embd}, 0);

                        ea_layer.ffn_gate = create_tensor(tn(LLM_TENSOR_FFN_GATE, "weight", i), {n_embd, n_ff}, 0);
                        ea_layer.ffn_down = create_tensor(tn(LLM_TENSOR_FFN_DOWN, "weight", i), {n_ff, n_embd}, 0);
                        ea_layer.ffn_up   = create_tensor(tn(LLM_TENSOR_FFN_UP,   "weight", i), {n_embd, n_ff}, 0);
                        ea_layer.ffn_norm = create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd}, 0);
    ...
    size_t n_filled_split_expert_tensors = 0;
    for (auto & ctx : pimpl->ctxs) {
        for (auto * cur = ggml_get_first_tensor(ctx.get()); cur != NULL; cur = ggml_get_next_tensor(ctx.get(), cur)) {
            if (cur->data == nullptr || ml.get_weight(ggml_get_name(cur)) != nullptr) {
                continue;
            }
            ...
            char type_str[64] = {};
            char suffix[32] = {};
            if (sscanf(name.c_str(), "blk.%d.%63[^.].%31s", &layer_id, type_str, suffix) != 3 || strcmp(suffix, "weight") != 0) {
                continue;
            }

            std::string split_type;
            if (strcmp(type_str, "ffn_gate_exps") == 0) {
                split_type = "ffn_gate";
            } else if (strcmp(type_str, "ffn_down_exps") == 0) {
                split_type = "ffn_down";
            } else if (strcmp(type_str, "ffn_up_exps") == 0) {
                split_type = "ffn_up";
            } else {
                continue;
            }

            for (int ie = 0; ie < cur->ne[2]; ++ie) {
                char split_name[256];
                snprintf(split_name, sizeof(split_name), "blk.%d.%s.%03d.weight", layer_id, split_type.c_str(), ie);

                const auto it = pimpl->ssd_experts.find(split_name);
                if (it == pimpl->ssd_experts.end()) {
                    throw std::runtime_error(format("missing split expert tensor info for %s", split_name));
                }

                const auto & info = it->second;
                GGML_ASSERT(info.file_idx < ml.files.size());
                GGML_ASSERT(info.size <= (size_t) cur->nb[2]);

                auto & file = ml.files.at(info.file_idx);
                file->seek(info.file_offs, SEEK_SET);
                file->read_raw((char *) cur->data + ie*cur->nb[2], info.size);
                n_filled_split_expert_tensors++;
            }
        }
    }
    if (n_filled_split_expert_tensors > 0) {
        LLAMA_LOG_INFO("%s: filled %zu split MoE expert tensors into CPU memory\n", __func__, n_filled_split_expert_tensors);
    }
    ...
    if (arch == LLM_ARCH_EAGLE && ea_layer.d2t != nullptr) {
        const int64_t n_draft_vocab = ea_layer.d2t->ne[0];
        if (ea_layer.d2t->type == GGML_TYPE_I64) {
            std::vector<int64_t> d2t_i64(n_draft_vocab);
            ggml_backend_tensor_get(ea_layer.d2t, d2t_i64.data(), 0, d2t_i64.size() * sizeof(int64_t));
            ea_layer.d2t_map.resize(n_draft_vocab);
            for (int64_t i = 0; i < n_draft_vocab; ++i) {
                ea_layer.d2t_map[i] = (int32_t) d2t_i64[i];
            }
        } else if (ea_layer.d2t->type == GGML_TYPE_I32) {
            ea_layer.d2t_map.resize(n_draft_vocab);
            ggml_backend_tensor_get(ea_layer.d2t, ea_layer.d2t_map.data(), 0, ea_layer.d2t_map.size() * sizeof(int32_t));
        } else {
            throw std::runtime_error("EAGLE3 d2t tensor must be i64 or i32");
        }

        ea_layer.t2d_map.assign(vocab.n_tokens(), 0);
        for (int64_t i = 0; i < n_draft_vocab; ++i) {
            const int64_t target_id = i + ea_layer.d2t_map[i];
            if (target_id >= 0 && target_id < (int64_t) ea_layer.t2d_map.size()) {
                ea_layer.t2d_map[target_id] = 1;
            }
        }
        LLAMA_LOG_INFO("%s: loaded EAGLE3 d2t tensor map (d2t: %zu, t2d: %zu)\n",
                       __func__, ea_layer.d2t_map.size(), ea_layer.t2d_map.size());
    }
...
void llama_model::print_info() const {
    ...
    // params is const; moe_reuse overrides handled at load time when provided
    ...
    if (!hparams.vocab_only) {
        ...
        if (hparams.moe_reuse_strength > 0.0f) {
            LLAMA_LOG_INFO("%s: moe_reuse_strength = %.3f\n", __func__, hparams.moe_reuse_strength);
            if (hparams.moe_reuse_expert_cap > 0) {
                LLAMA_LOG_INFO("%s: moe_reuse_expert_cap = %d\n", __func__, hparams.moe_reuse_expert_cap);
            }
        }
...
struct llm_build_qwen3 : public llm_graph_context {
    llm_build_qwen3(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        // EAGLE3: Store token embeddings for draft model usage (similar to hidden states)
        res->t_embd = inpL;
        cb(inpL, "target_token_embeddings", -1);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // EAGLE3 Multi-Layer Hidden States Capture for Qwen3 (SGLang-compatible: capture pre-attention)
            // Capture layers [2, n_layer/2, n_layer-3] for EAGLE3
            if (hparams.n_layer >= 35 && res != nullptr) {
                static const std::vector<int> eagle_target_layers = {2, (int)(hparams.n_layer/2), (int)(hparams.n_layer-3)};

                for (size_t extract_idx = 0; extract_idx < eagle_target_layers.size(); ++extract_idx) {
                    if (il == eagle_target_layers[extract_idx]) {
                        // Use ggml_dup to create an independent copy of layer input (pre-attention)
                        ggml_tensor * captured_hs = ggml_dup(ctx0, inpSA);
                        const std::string extract_name = "eagle3_extract_" + std::to_string(extract_idx);
                        cb(captured_hs, extract_name.c_str(), il);

                        // Build forward to ensure the dup operation is executed
                        ggml_build_forward_expand(gf, captured_hs);
        ...
        // EAGLE3: Store final hidden states for target model guidance
        res->t_hidden_states = cur;
        cb(cur, "target_hidden_states", -1);
...
struct llm_build_qwen3moe : public llm_graph_context {
    llm_build_qwen3moe(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        auto parse_skip_layers = [](const char * env, int32_t n_layer) {
            std::vector<char> skip(n_layer, false);
        ...
        const bool is_draft_moe_context = cparams.is_draft_context;
        const std::vector<char> skip_attn = is_draft_moe_context ? parse_skip_layers(getenv("LLAMA_DRAFT_SKIP_ATTN"), n_layer) : std::vector<char>(n_layer, false);
        const std::vector<char> skip_mlp  = is_draft_moe_context ? parse_skip_layers(getenv("LLAMA_DRAFT_SKIP_MLP"),  n_layer) : std::vector<char>(n_layer, false);
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (res != nullptr) {
                for (size_t extract_idx = 0; extract_idx < cparams.eagle3_extract_layers.size(); ++extract_idx) {
                    if (il == cparams.eagle3_extract_layers[extract_idx]) {
                        ggml_tensor * captured_hs = ggml_dup(ctx0, inpSA);
                        const std::string extract_name = "eagle3_extract_" + std::to_string(extract_idx);
                        cb(captured_hs, extract_name.c_str(), il);
                        ggml_build_forward_expand(gf, captured_hs);
            ...
            ggml_tensor * ffn_inp = attn_out ? ggml_add(ctx0, attn_out, inpSA) : inpSA;
            cb(ffn_inp, "ffn_inp", il);
...
struct llm_build_deepseek2 : public llm_graph_context {
    llm_build_deepseek2(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        ggml_tensor * inp_out_ids = build_inp_out_ids();
        int64_t actual_n_layers;
        if (hparams.dft_exit_layer > 0) {
            actual_n_layers = (n_tokens==1 ? 27 : n_layer);
            // LLAMA_LOG_INFO("Using DFT exit layer %d for n_tokens=%d\n", (int)actual_n_layers, (int)n_tokens);
        }
        else {
            actual_n_layers = n_layer;
        }

        for (int il = 0; il < actual_n_layers; ++il) {
            ggml_tensor * inpSA = inpL;
            ...
            if (il == actual_n_layers - 1 && inp_out_ids) {
                cur   = ggml_get_rows(ctx0,   cur, inp_out_ids);
                inpSA = ggml_get_rows(ctx0, inpSA, inp_out_ids);
            }
            ...
            if ((uint32_t) il < hparams.n_layer_dense_lead) {
                ...
                // MoE branch
                ggml_tensor * moe_out;
                if (n_tokens==1 and false) {
                    ggml_tensor * zero_tensor = ggml_new_tensor_2d(ctx0, cur->type, n_embd, 1);
                    ggml_set_zero(zero_tensor);
                    moe_out =zero_tensor;
                }
                else {
                    moe_out =
...
struct llm_build_openai_moe_iswa : public llm_graph_context {
    llm_build_openai_moe_iswa(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            // EAGLE3 layer IDs index the target hidden-state sequence. IDs below
            // n_layer are inputs to that layer; n_layer is the final layer output.
            if (res != nullptr) {
                for (size_t extract_idx = 0; extract_idx < cparams.eagle3_extract_layers.size(); ++extract_idx) {
                    if (il == cparams.eagle3_extract_layers[extract_idx]) {
                        ggml_tensor * captured_hs = ggml_dup(ctx0, inpSA);
        ...
        if (res != nullptr) {
            for (size_t extract_idx = 0; extract_idx < cparams.eagle3_extract_layers.size(); ++extract_idx) {
                if (cparams.eagle3_extract_layers[extract_idx] == n_layer) {
                    ggml_tensor * captured_hs = ggml_dup(ctx0, cur);
                    const std::string extract_name = "eagle3_extract_" + std::to_string(extract_idx);
                    cb(captured_hs, extract_name.c_str(), n_layer);
                    ggml_build_forward_expand(gf, captured_hs);
                    break;
                }
            }
...
struct llm_build_eagle3_encode : public llm_graph_context {
    llm_build_eagle3_encode(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        auto inp_target = std::make_unique<llm_graph_input_embd>();
        inp_target->embd = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, 3 * hparams.eagle3_target_hidden_size, n_tokens);
        ...
        if (model.ea_layer.input_norm) {
            cur = build_norm(cur, model.ea_layer.input_norm, nullptr, LLM_NORM_RMS, -1);
            cb(cur, "input_norm", -1);
        }
        cur = build_lora_mm(model.ea_layer.fc, cur);
        cb(cur, "fc_out", -1);
...
struct llm_build_eagle3_decode : public llm_graph_context {
    llm_build_eagle3_decode(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;

        GGML_ASSERT(n_embd_head == hparams.n_embd_head_k);
        GGML_ASSERT(n_layer == 1);

        ggml_tensor * token_embd = model.ea_layer.tok_embd ? model.ea_layer.tok_embd : shared_token_embd;
        GGML_ASSERT(token_embd != nullptr && "EAGLE3 decoder requires own or shared token embeddings");

        ggml_tensor * inp_embd = build_inp_embd(token_embd);
        ...
        ggml_tensor * inpSA = inp_g;

        ggml_tensor * embd_norm = build_norm(inp_embd,
                model.ea_layer.attn_norm, NULL,
                LLM_NORM_RMS, il);
        cb(embd_norm, "embd_norm", il);

        ggml_tensor * g_norm = build_norm(inp_g,
                model.ea_layer.attn_norm_2, NULL,
                LLM_NORM_RMS, -1);
        cb(g_norm, "g_norm", il);

        ggml_tensor * cur = ggml_concat(ctx0, embd_norm, g_norm, il);
        cb(cur, "concat_embd", il);

        ggml_tensor * Qcur = build_lora_mm(model.ea_layer.wq, cur);
        cb(Qcur, "Qcur", il);

        ggml_tensor * Kcur = build_lora_mm(model.ea_layer.wk, cur);
        cb(Kcur, "Kcur", il);

        ggml_tensor * Vcur = build_lora_mm(model.ea_layer.wv, cur);
        cb(Vcur, "Vcur", il);
        ...
        cur = build_norm(ffn_inp,
                model.ea_layer.ffn_norm, NULL,
                LLM_NORM_RMS, il);
        cb(cur, "post_attn_norm", il);

        cur = build_ffn(cur,
                model.ea_layer.ffn_up,   NULL, NULL,
                model.ea_layer.ffn_gate, NULL, NULL,
                model.ea_layer.ffn_down, NULL, NULL,
                NULL,
                LLM_FFN_SILU, LLM_FFN_PAR, il);
        cb(cur, "ffn_out", il);
        ...
        cur = build_norm(cur,
                model.ea_layer.output_norm, NULL,
                LLM_NORM_RMS, -1);
        cb(cur, "result_norm", -1);

        cur = build_lora_mm(model.ea_layer.output, cur);
...
struct llm_build_eagle3 : public llm_graph_context {
    llm_build_eagle3(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        ...
        // 1. Use target model embeddings via shared tensor (SGLang style)
        // EAGLE3 uses target model's token embedding layer for lookups
        // Create placeholder input tensor that will be replaced when shared_token_embd is set
        ggml_tensor * target_embeddings_input = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_embd, n_tokens);
        ggml_set_input(target_embeddings_input);
        ggml_set_name(target_embeddings_input, "target_model_embeddings");

        if (shared_token_embd) {
            LLAMA_LOG_DEBUG("EAGLE3: Using shared token embedding from target model\n");
            inpL = build_inp_embd(shared_token_embd);
            cb(inpL, "target_token_embeddings", -1);

            // DEBUG: Also access token IDs for debugging
            if (res->t_tokens) {
                cb(res->t_tokens, "inp_tokens", -1);
                LLAMA_LOG_DEBUG("EAGLE3: Added inp_tokens callback for token ID debugging\n");
            }
        } else {
            LLAMA_LOG_DEBUG("EAGLE3: Creating placeholder embedding input - will be replaced when shared_token_embd is set\n");
            inpL = target_embeddings_input;
            cb(inpL, "target_model_embeddings", -1);
        }
        ...
        // EAGLE3: Target hidden states processing (original architecture)
        // This creates the target HS pathway: 7680→FC→2560→norm→concat
        // Note: Draft HS pathway will be added as a separate improvement

        // 2. Create target hidden states input tensor (will be set by llama_context_set_target_hidden_states)
        // This corresponds to 3 layers * 2560 dims = 7680 total dims from target model
        // Create as external input that will be populated at runtime
        ggml_tensor * target_hs = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_embd * 3, n_tokens);
        ggml_set_input(target_hs);  // Mark as external input
        ggml_set_name(target_hs, "target_hidden_states");
        // Note: This input will be populated by llama_context_set_target_hidden_states() at runtime

        // EAGLE3: Create draft hidden states input tensor for addition-based solution
        // This corresponds to 2560 dims from draft model's own last layer
        // Create as external input that will be populated at runtime
        // Step 0: initialized to zeros (no effect on target_hs_proj)
        // Step 1: populated with draft HS (adds to zeroed target_hs_proj)
        ggml_tensor * draft_hs = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_embd, n_tokens);
        ggml_set_input(draft_hs);  // Mark as external input
        ggml_set_name(draft_hs, "draft_hidden_states");
        // Note: This input will be populated by llama_context_set_draft_hidden_states() at runtime
        // Initialization to zeros will be handled in graph_compute

        // Log tensor dimensions for debugging
        if (!model.ea_layer.fc) {
            LLAMA_LOG_ERROR("EAGLE3: ERROR - model.ea_layer.fc is NULL!\n");
        }
        LLAMA_LOG_DEBUG("EAGLE3: target_hs dims [%ld, %ld, %ld, %ld]\n",
                target_hs->ne[0], target_hs->ne[1], target_hs->ne[2], target_hs->ne[3]);
        LLAMA_LOG_DEBUG("EAGLE3: draft_hs dims [%ld, %ld, %ld, %ld]\n",
                draft_hs->ne[0], draft_hs->ne[1], draft_hs->ne[2], draft_hs->ne[3]);
        LLAMA_LOG_DEBUG("EAGLE3: ea_layer.fc dims [%ld, %ld, %ld, %ld]\n",
                model.ea_layer.fc->ne[0], model.ea_layer.fc->ne[1],
                model.ea_layer.fc->ne[2], model.ea_layer.fc->ne[3]);

        // 3. Draft model processes target hidden states with FC layer (for Step 0)
        // For Step 1&2, draft_hidden_states will be used directly (2560 dims)
        ggml_tensor * target_hs_fc_in = target_hs;
        if (model.ea_layer.input_norm) {
            target_hs_fc_in = build_norm(
                    target_hs_fc_in, model.ea_layer.input_norm, nullptr, LLM_NORM_RMS, -1);
            cb(target_hs_fc_in, "target_hs_input_norm", -1);
        }
        ggml_tensor * target_hs_proj = ggml_mul_mat(ctx0, model.ea_layer.fc, target_hs_fc_in);
        cb(target_hs_proj, "target_hs_proj", -1);

        // EAGLE3: Also add draft_hs to callback for graph construction
        cb(draft_hs, "draft_hidden_states", -1);

        ggml_tensor * embeds_norm = build_norm(inpL,
                model.ea_layer.attn_norm, NULL,
                LLM_NORM_RMS, 0);
        cb(embeds_norm, "embeds_norm", -1);

        // EAGLE3: Add target_hs_proj and draft_hs BEFORE applying hidden_norm (matching SGLang)
        ggml_tensor * target_hs_sum = ggml_add(ctx0, target_hs_proj, draft_hs);
        cb(target_hs_sum, "target_hs_sum", -1);

        // Apply hidden_norm AFTER summing (correct order)
        ggml_tensor * target_hs_norm = build_norm(target_hs_sum,
                model.ea_layer.attn_norm_2, NULL,
                LLM_NORM_RMS, 0);
        cb(target_hs_norm, "target_hs_norm", -1);

        ggml_tensor * fused_hs_norm = target_hs_norm;  // Already normalized
        ggml_set_name(fused_hs_norm, "fused_hs_norm");
        cb(fused_hs_norm, "fused_hs_norm", -1);

        // 5. Input fusion: [embeds; fused_hidden_states] -> 5120 dimensions (2560 + 2560)
        ggml_tensor * fused_input = ggml_concat(ctx0, embeds_norm, fused_hs_norm, 0);  // (batch_size, 5120)
        cb(fused_input, "fused_input", -1);

        // 6. Process through single EAGLE3 transformer layer
        // EAGLE3 has only one layer, so no loop needed
        {
            // EAGLE3 official implementation:
            // residual = target_hs_sum (target_hs_proj + draft_hs, 2560维)
            // For Step 0: draft_hs=0, residual=target_hs_proj
            // For Step 1+: draft_hs=actual draft HS, residual=target_hs_proj+draft_hs
            ggml_tensor * residual = target_hs_sum;
            cb(residual, "residual", 0);

            // Note: fused_input is already normalized above, no need to normalize again
            cur = fused_input;
            cb(cur, "fused_input_normed", 0);

            // self-attention (with widened input dimensions)
            {
                // compute Q and K and RoPE them
                LLAMA_LOG_DEBUG("EAGLE3: About to compute Qcur...\n");
                LLAMA_LOG_DEBUG("EAGLE3: wq dims [%ld, %ld], cur dims [%ld, %ld]\n",
                        model.ea_layer.wq->ne[0], model.ea_layer.wq->ne[1],
                        cur->ne[0], cur->ne[1]);
                ggml_tensor * Qcur = build_lora_mm(model.ea_layer.wq, cur);
                cb(Qcur, "Qcur", 0);

                LLAMA_LOG_DEBUG("EAGLE3: About to compute Kcur...\n");
                ggml_tensor * Kcur = build_lora_mm(model.ea_layer.wk, cur);
                cb(Kcur, "Kcur", 0);

                LLAMA_LOG_DEBUG("EAGLE3: About to compute Vcur...\n");
                ggml_tensor * Vcur = build_lora_mm(model.ea_layer.wv, cur);
                cb(Vcur, "Vcur", 0);

                // Reshape for attention - note that attention heads expect the widened dimensions
                Qcur = ggml_reshape_3d(ctx0, Qcur, n_embd_head, n_head,    n_tokens);
                Kcur = ggml_reshape_3d(ctx0, Kcur, n_embd_head, n_head_kv, n_tokens);
                Vcur = ggml_reshape_3d(ctx0, Vcur, n_embd_head, n_head_kv, n_tokens);

                // Note: attn_norm_2 has already been applied to hidden_states in hs_norm (line ~19191)
                // Following sglang's architecture, no additional normalization is needed for Q/K/V
                cb(Qcur, "Qcur", 0);
                ...
                cur = build_attn(inp_attn,
                        model.ea_layer.wo, NULL,  // EAGLE3 doesn't have bias
            ...
            // EAGLE3 First Residual Connection:
            // Attention output is already 2560 dims (wo projects from 4096 to 2560)
            // Add attention output to residual (target_hs_proj)
            cur = ggml_add(ctx0, cur, residual);
            cb(cur, "after_first_residual", 0);

            // Update residual for second residual connection
            residual = cur;

            // feed-forward network
            // EAGLE3: Apply post-attention layernorm (post_attention_layernorm)
            cur = build_norm(cur,
                    model.ea_layer.ffn_norm, NULL,
                    LLM_NORM_RMS, 0);
            cb(cur, "ffn_norm", 0);

            LLAMA_LOG_DEBUG("EAGLE3: Before ffn_up - cur dims [%ld, %ld], ffn_up dims [%ld, %ld]\n",
                    cur->ne[0], cur->ne[1],
                    model.ea_layer.ffn_up->ne[0], model.ea_layer.ffn_up->ne[1]);
            LLAMA_LOG_DEBUG("EAGLE3: ffn_gate dims [%ld, %ld], ffn_down dims [%ld, %ld]\n",
                    model.ea_layer.ffn_gate->ne[0], model.ea_layer.ffn_gate->ne[1],
                    model.ea_layer.ffn_down->ne[0], model.ea_layer.ffn_down->ne[1]);
            cur = build_ffn(cur,
                    model.ea_layer.ffn_up,   NULL, NULL,
                    model.ea_layer.ffn_gate, NULL, NULL,
                    model.ea_layer.ffn_down, NULL, NULL,
            ...
            // EAGLE3 Second Residual Connection
            cur = ggml_add(ctx0, cur, residual);
            cb(cur, "after_second_residual", 0);
        }

        // 7. Final output processing
        // Following sglang: apply post-attention norm before output
        // This should handle the 5120->2560 dimension reduction
        res->t_hidden_states = cur;  // For continuous speculative decoding [先设置为norm之前]
        res->t_embd = cur;
        cur = build_norm(cur,
                model.ea_layer.output_norm, NULL,
                LLM_NORM_RMS, -1);
        ...
        // 8. lm_head - generate draft logits (32000 vocab size)
        cur = build_lora_mm(model.ea_layer.output, cur);

        cb(cur, "result_output", -1);
        res->t_logits = cur;

        // EAGLE3: Add callback to fill external input tensors with target model data
        ggml_build_forward_expand(gf, cur);
    }
};
        ...
        case LLM_ARCH_EAGLE:
            {
                ...
                } else if (params.gtype == LLM_GRAPH_TYPE_DECODER) {
...
llama_model_params llama_model_default_params() {
    llama_model_params result = {
        ...
        /*.gpu_experts_json            =*/ nullptr,
        /*.progress_callback           =*/ nullptr,
        /*.progress_callback_user_data =*/ nullptr,
        /*.kv_overrides                =*/ nullptr,
        /*.moe_reuse_strength          =*/ -1.0f,
        /*.moe_reuse_expert_cap        =*/ -1,
        /*.n_expert_used               =*/ -1,
        /*.dft_exit_layer              =*/ -1,
        /*.vocab_only                  =*/ false,
        /*.use_mmap                    =*/ true,
...
std::map<std::string, ssd_expert_info_t> llama_model::get_ssd_experts() const {
    // 这里的 pimpl 是可见的
    std::map<std::string, ssd_expert_info_t> result;
    for (const auto& kv : pimpl->ssd_experts) {
        // 需要把 ssd_expert_info (impl内部定义的) 转换成 ssd_expert_info_t (外部定义的)
        // 为了省事，建议统一用同一个结构体定义，或者在这里手动拷贝字段
        ssd_expert_info_t info;
        info.file_idx = kv.second.file_idx;
        info.file_offs = kv.second.file_offs;
        info.size = kv.second.size;
        info.type = kv.second.type;
        info.ne = kv.second.ne;
        result[kv.first] = info;
    }
    return result;
}

...
struct eagle_layer {
    ...
    struct ggml_tensor * fc;   // fully connected layer
    struct ggml_tensor * fc_b; // fc bias
    struct ggml_tensor * input_norm;

    struct ggml_tensor * output;   // output layer

    struct ggml_tensor * attn_norm;
    struct ggml_tensor * attn_norm_2;
    struct ggml_tensor * output_norm;

    struct ggml_tensor * d2t; // new EAGLE3 GGUF tensor-backed draft-to-target map

    // vocabulary mappings (metadata, not tensors)
    std::vector<int32_t> d2t_map;
    std::vector<uint8_t> t2d_map;
};
...
struct llama_model {
    ...
    std::map<std::string, ssd_expert_info_t> get_ssd_experts() const;
private:
    struct impl;

...
class Eagle3Model(Qwen3Model):
    model_arch = gguf.MODEL_ARCH.EAGLE3_DRAFT

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        hparams = ModelBase.load_hparams(self.dir_model, False)
        self.draft_vocab_size = hparams.get("draft_vocab_size", 32000)
        self.origin_hf_arch = hparams.get('architectures', [None])[0]
        self.eagle_extract_layers = hparams.get(
            "eagle_aux_hidden_state_layer_ids",
            hparams.get("eagle_config", {}).get("eagle_aux_hidden_state_layer_ids"),
        )
    ...
    def set_gguf_parameters(self):
        super().set_gguf_parameters()
        if self.eagle_extract_layers is not None:
            self.gguf_writer.add_array("eagle3.extract_layers", self.eagle_extract_layers)
        self.gguf_writer.add_uint32("eagle3.target_hidden_size", self.hparams["hidden_size"])
        self.gguf_writer.add_bool("eagle3.norm_before_fc", self.hparams.get("norm_before_fc", False))

        from safetensors import safe_open

        model_name = "model.safetensors"
        model_path = self.dir_model / model_name
        if model_path.exists():
            with safe_open(model_path, framework="pt", device="cpu") as f:
                if "d2t" not in f.keys():
                    self.d2t_diff = np.zeros(self.draft_vocab_size, dtype=np.int32)
                    self.t2d_map = np.ones(self.hparams["vocab_size"], dtype=np.bool_)
                    self.gguf_writer.add_eagle_d2t_map(self.d2t_diff.tolist())
                    self.gguf_writer.add_eagle_t2d_map(self.t2d_map.tolist())
                    print(f"EAGLE3: using identity full-vocabulary mapping ({self.draft_vocab_size} tokens)")
                    return

                # 加载d2t映射 - 修复：使用numpy数组确保数据完整性
                d2t_tensor = f.get_tensor('d2t')
                d2t_data = d2t_tensor.to(torch.int32).numpy()
                print(f"EAGLE3 DEBUG: d2t tensor shape={d2t_tensor.shape}, dtype={d2t_tensor.dtype}")
                print(f"EAGLE3 DEBUG: d2t first 10 values: {d2t_tensor[:10].tolist()}")

                # 修复：确保正确转换为Python list后再保存为metadata
                d2t_list = d2t_tensor.to(torch.int32).tolist()
                print(f"EAGLE3 DEBUG: d2t list length={len(d2t_list)}")
                print(f"EAGLE3 DEBUG: d2t first 10 values: {d2t_list[:10]}")

                # 将D2T列表分成小批次保存以避免内存问题（保存差值，而不是绝对值）
                self.gguf_writer.add_eagle_d2t_map(d2t_list)

                # 保存d2t差值供后续vocab构建使用
                self.d2t_diff = d2t_tensor.to(torch.int32).numpy()

                print(f"EAGLE3: saved d2t_diff for vocab building")

                # 加载t2d映射 - 修复：保持为bool列表以匹配GGUF_TYPE_BOOL
                t2d_tensor = f.get_tensor('t2d') if "t2d" in f.keys() else None
                if t2d_tensor is None:
                    self.t2d_map = np.ones(self.hparams["vocab_size"], dtype=np.bool_)
                    self.gguf_writer.add_eagle_t2d_map(self.t2d_map.tolist())
                    return
                print(f"EAGLE3 DEBUG: t2d tensor shape={t2d_tensor.shape}, dtype={t2d_tensor.dtype}")

                # 转换为bool列表（True/False）而不是int列表
                t2d_list = t2d_tensor.to(torch.bool).tolist()
                print(f"EAGLE3 DEBUG: t2d list length={len(t2d_list)}")
                print(f"EAGLE3 DEBUG: t2d first 10 values: {t2d_list[:10]}")

                # 保存映射供后续vocab构建使用
                self.t2d_map = t2d_tensor.to(torch.bool).numpy()

                # 直接存储为bool数组
                self.gguf_writer.add_eagle_t2d_map(t2d_list)

                print(f"Added EAGLE vocabulary mappings: d2t_map({len(d2t_list)}), t2d_map({len(t2d_list)})")

    def set_vocab(self):
        import json
        from pathlib import Path

        # 1. 获取Target的真实vocab size (151643)，而不是draft_vocab_size (32000)
        target_vocab_size = self.hparams.get("vocab_size")
        if not target_vocab_size:
            target_vocab_size = 151643  # Qwen2.5 default
        print(f"EAGLE: Setting up vocabulary. Target vocab size is {target_vocab_size}.")

        # 2. 直接读取 tokenizer.json 文件获取词汇表
        tokenizer_path = Path(self.dir_model) / "tokenizer.json"
        if not tokenizer_path.is_file():
            print(f"FATAL: tokenizer.json not found in {self.dir_model}.")
            print("Please ensure tokenizer.json from the main Qwen3-4B-Instruct-2507 model is in this directory.")
        ...
        # 3. 从 tokenizer.json 内部提取词汇表
        vocab_data = tokenizer_data.get("model", {}).get("vocab", {})
        if not vocab_data:
            raise ValueError("Could not find 'vocab' section in tokenizer.json")

        id_to_token = {v: k for k, v in vocab_data.items()}

        # 4. 【EAGLE3架构修复】使用正确的Draft vocab (32k)构建GGUF词汇表
        # EAGLE3的输出能力就是32k，诚实在GGUF header中反映这一点
        tokens = [id_to_token.get(i, f"<unk_{i}>") for i in range(self.draft_vocab_size)]
        print(f"EAGLE3: Using Draft vocab (first {self.draft_vocab_size} tokens)")
        print(f"EAGLE3: First 10 tokens: {tokens[:10]}")
        print(f"EAGLE3: Last 10 tokens: {tokens[-10:]}")

        toktypes = [gguf.TokenType.NORMAL] * self.draft_vocab_size

        # 5. 写入 GGUF Header - 诚实地告诉llama.cpp我们有32k个词的输出能力
        self.gguf_writer.add_tokenizer_model("gpt2")
        self.gguf_writer.add_tokenizer_pre("qwen2")
        self.gguf_writer.add_token_list(tokens)
        self.gguf_writer.add_token_types(toktypes)
        self.gguf_writer.add_vocab_size(self.draft_vocab_size)

        # 6. 记录Target和Draft的vocab size作为元数据，供运行时使用
        self.gguf_writer.add_uint32("eagle.draft_vocab_size", self.draft_vocab_size)
        self.gguf_writer.add_uint32("eagle.target_vocab_size", target_vocab_size)
        print(f"EAGLE3: Recorded draft vocab size: {self.draft_vocab_size}")
        print(f"EAGLE3: Recorded target vocab size: {target_vocab_size}")

        # 5. 使用 SpecialVocab 处理 merges 和特殊tokens（这是正确的方式）
        print("Loading special vocabulary (merges and special tokens)...")
        special_vocab = gguf.SpecialVocab(self.dir_model, load_merges=True)
        special_vocab.add_to_gguf(self.gguf_writer)

        # 7. 手动添加特殊 token ID
        added_tokens = tokenizer_data.get("added_tokens", [])
        special_tokens = {tok['content']: tok['id'] for tok in added_tokens if 'content' in tok and 'id' in tok}

        eos_id = special_tokens.get("<|im_end|>", None)
        if eos_id is not None:
            # 直接设置EOS token，因为我们现在使用完整的Target vocab
            self.gguf_writer.add_eos_token_id(eos_id)
            print(f"EAGLE3: Set EOS token ID to {eos_id}")

        print(f"Successfully added vocabulary ({len(tokens)} tokens) with merges and special tokens.")

    
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        # EAGLE3特殊的tensor映射
        if name in ("d2t", "t2d"):
            # d2t和t2d现在作为元数据存储，不再作为tensor处理
            # 跳过这些tensor，因为它们已经在set_gguf_parameters中作为元数据添加
            return

        # 处理EAGLE3的权重绑定
        if name == "lm_head.weight":
            # 只写入output.weight (32k x Dim)
            # 绝对不要写入token_embd.weight，因为：
            # 1. EAGLE3使用Shared Embedding，没有自己的Embedding权重
            # 2. 如果写入32k的token_embd但GGUF header说是151643，会维度报错
            # 3. 这个token_embd在推理时根本没用（因为我们会调用set_target_embedding_layer）
            yield ("output.weight", data_torch)
            return

        if name in ("fc.weight", "input_norm.weight"):
        ...
        # 处理EAGLE3特殊的tensor名称映射
        if name.startswith("midlayer."):
            # 将midlayer.*映射到标准的llama.cpp格式
            base_name = name.replace("midlayer.", "")

            # 映射到标准的tensor名称
            mapping = {
                "hidden_norm.weight": "blk.0.hidden_norm.weight",
                "input_layernorm.weight": "blk.0.attn_norm.weight",
                "mlp.down_proj.weight": "blk.0.ffn_down.weight",
                "mlp.gate_proj.weight": "blk.0.ffn_gate.weight",
                "mlp.up_proj.weight": "blk.0.ffn_up.weight",
                "post_attention_layernorm.weight": "blk.0.ffn_norm.weight",
                "self_attn.k_proj.weight": "blk.0.attn_k.weight",
                "self_attn.o_proj.weight": "blk.0.attn_output.weight",
                "self_attn.q_proj.weight": "blk.0.attn_q.weight",
                "self_attn.v_proj.weight": "blk.0.attn_v.weight",
                "norm.weight": "output_norm.weight"
            }

            new_name = mapping.get(base_name, f"blk.0.{base_name}")
            yield (new_name, data_torch)
            return
...
class DeepseekModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.find("mlp.experts") != -1:
            ...
            match_expert = re.match(r"model\.layers\.(\d+)\.mlp\.experts\.(\d+)\.(down_proj|gate_proj|up_proj)\.weight", name)
            if match_expert is None:
                raise ValueError(f"Unrecognized DeepSeek expert tensor name: {name}")

            layer_id = int(match_expert.group(1))
            expert_id = int(match_expert.group(2))
            w_name = match_expert.group(3)
            if expert_id < 0 or expert_id >= n_experts:
                raise ValueError(f"DeepSeek expert id out of range in tensor name: {name}")
            ...
            return [(f"blk.{layer_id}.{split_type}.{expert_id:03d}.weight", data_torch)]

...
#endif
#include "llama.h"
// extern void llama_clear_ssd_cache_backend();
...
int main(int argc, char ** argv) {
    ...
    // Dataset mode: load multiple prompts from a JSONL file and run in a single model session.
    if (!params.dataset_path.empty()) {
        if (params.interactive || params.interactive_first) {
            LOG_WRN("%s: dataset mode forces non-interactive execution\n", __func__);
            params.interactive = false;
            params.interactive_first = false;
        }
        if (!params.prompt.empty()) {
            LOG_WRN("%s: --prompt is ignored in dataset mode\n", __func__);
        }
        ...
        LOG_INF("dataset mode: n_predict = %d, n_batch = %d, n_ctx = %d\n", params.n_predict, params.n_batch, llama_n_ctx(ctx));

        const bool use_chat_template = params.enable_chat_template && has_chat_template;
        const int n_ctx = llama_n_ctx(ctx);
        ...
        while (std::getline(dataset_file, json_line)) {
            if (json_line.empty()) {
                continue;
            }
            ...
            std::vector<llama_token> embd_inp = common_tokenize(ctx, current_prompt, true, true);
            if ((int) embd_inp.size() > n_ctx - 4) {
                LOG_WRN("%s: prompt too long (%d tokens, max %d); skipping\n", __func__, (int) embd_inp.size(), n_ctx - 4);
                continue;
            }
            ...
            const bool stop_at_eog = params.n_predict < 0;
            if (params.n_predict < 0) {
                n_remain = n_ctx - n_past - 1;
                if (n_remain < 0) {
                    n_remain = 0;
                }
            }
            ...
            while (n_remain != 0) {
                ...
                if (stop_at_eog && llama_vocab_is_eog(vocab, id)) {
                    break;
                }
                if (!params.ctx_shift && n_past >= n_ctx - 1) {
            ...
            const double t_enc_s = (t_enc_end - t_enc_start) / 1e6;
            const double t_dec_s = (t_dec_end - t_dec_start) / 1e6;
            const int n_input = (int) embd_inp.size();

            LOG("\n\n");
            LOG_INF("Processed Question %d in %.3f s\n", question_idx, (t_dec_end - t_dec_start) / 1e6f);
            LOG_INF("encoded %4d tokens in %8.3f seconds, speed: %8.3f t/s\n",
                    n_input, t_enc_s, t_enc_s > 0.0 ? (n_input / t_enc_s) : 0.0);
            LOG_INF("decoded %4d tokens in %8.3f seconds, speed: %8.3f t/s\n",
                    n_predict, t_dec_s, t_dec_s > 0.0 ? (n_predict / t_dec_s) : 0.0);
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if ((int) embd_inp.size() <= n_consumed && !is_interacting) {
            ...
            // [ADDED] Start timer when generation (decoding) begins            
            if (t_dec_start == 0) {
    ...
    LOG("\n\n");
     ...
     // [ADDED] Print end-to-end throughput stats matching speculative.cpp style
    {
        auto t_dec_end = ggml_time_us();
        auto n_predict_gen = output_tokens.size();
        if (n_predict_gen > 0 && t_dec_start > 0) {
            LOG_INF("\ndecoded %4d tokens in %8.3f seconds, speed: %8.3f t/s\n", 
                (int)n_predict_gen, 
                (t_dec_end - t_dec_start) / 1e6f, 
                n_predict_gen / ((t_dec_end - t_dec_start) / 1e6f));
        } 
    } 

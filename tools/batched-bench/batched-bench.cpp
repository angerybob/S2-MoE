...
int main(int argc, char ** argv) {
    ...
    if (params.moe_expert_topk > 0) {
        llama_context_set_moe_expert_topk(ctx, params.moe_expert_topk);
    }
    ...
    for (        int i_pp = 0; i_pp < (int) n_pp.size(); ++i_pp) {
        for (    int i_tg = 0; i_tg < (int) n_tg.size(); ++i_tg) {
            for (int i_pl = 0; i_pl < (int) n_pl.size(); ++i_pl) {
                ...
                if(params.batched_bench_output_jsonl) {
                    const float tpot_ms = (pl > 0 && tg > 0) ? (t_tg * 1000.0f) / float(pl * tg) : 0.0f;
                    LOG(
                        "{\"n_kv_max\": %d, \"n_batch\": %d, \"n_ubatch\": %d, \"flash_attn\": %d, \"is_pp_shared\": %d, \"n_gpu_layers\": %d, \"n_threads\": %u, \"n_threads_batch\": %u, "
                        "\"moe_expert_topk\": %d, \"pp\": %d, \"tg\": %d, \"pl\": %d, \"n_kv\": %d, \"t_pp\": %f, \"speed_pp\": %f, \"t_tg\": %f, \"speed_tg\": %f, \"t\": %f, \"speed\": %f, \"tpot_ms\": %f}\n",
                        ...
                        params.moe_expert_topk, pp, tg, pl, n_kv, t_pp, speed_pp, t_tg, speed_tg, t, speed, tpot_ms
                    );
                } else {

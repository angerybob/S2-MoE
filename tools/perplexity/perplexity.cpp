...
static std::vector<std::pair<std::string, std::string>> load_answer_ppl_tsv(const std::string & prompt) {
    ...
    while (std::getline(in, line)) {
        ...
        if (tab == std::string::npos) {
            LOG_ERR("%s: bad answer-ppl row without tab separator\n", __func__);
            continue;
        }
        result.emplace_back(json_unescape(line.substr(0, tab)), json_unescape(line.substr(tab + 1)));
    }
    return result;
}

static void answer_ppl_score(llama_context * ctx, const common_params & params) {
    const llama_model * model = llama_get_model(ctx);
    const llama_vocab * vocab = llama_model_get_vocab(model);

    const auto samples = load_answer_ppl_tsv(params.prompt);
    if (samples.empty()) {
        LOG_ERR("%s: no answer-ppl samples\n", __func__);
        return;
    }
    ...
    const int bos     = llama_vocab_bos(vocab);
    ...
    for (size_t is = 0; is < samples.size(); ++is) {
        std::vector<llama_token> prompt_tokens = common_tokenize(ctx, samples[is].first, add_bos, params.parse_special);
        std::vector<llama_token> answer_tokens = common_tokenize(ctx, samples[is].second, false, params.parse_special);
        if (prompt_tokens.empty() || answer_tokens.empty()) {
            continue;
        }
        if ((int) prompt_tokens.size() + (int) answer_tokens.size() > n_ctx) {
            const int keep_prompt = n_ctx - (int) answer_tokens.size();
            if (keep_prompt <= 1) {
                LOG_WRN("%s: skip sample %zu because answer does not fit context\n", __func__, is);
                continue;
            }
            std::vector<llama_token> truncated;
            truncated.reserve(keep_prompt);
            if (add_bos) {
                truncated.push_back(bos);
            }
            const int tail = keep_prompt - (int) truncated.size();
            truncated.insert(truncated.end(), prompt_tokens.end() - tail, prompt_tokens.end());
            prompt_tokens.swap(truncated);
        }

        std::vector<llama_token> tokens = prompt_tokens;
        tokens.insert(tokens.end(), answer_tokens.begin(), answer_tokens.end());
        const int n_tokens = (int) tokens.size();
        const int answer_start = (int) prompt_tokens.size();

        llama_batch batch = llama_batch_init(n_tokens, 0, 1);
        common_batch_clear(batch);
        int n_outputs = 0;
        for (int i = 0; i < n_tokens; ++i) {
            const bool need_logits = i >= answer_start - 1 && i < n_tokens - 1;
        ...
        if (!decode_helper(ctx, batch, batch_logits, n_batch, n_vocab)) {
            LOG_ERR("%s: llama_decode() failed on sample %zu\n", __func__, is);
        ...
        eval_pairs.clear();
        size_t logit_idx = 0;
        for (int i = answer_start - 1; i < n_tokens - 1; ++i) {
            eval_pairs.emplace_back(logit_idx++, tokens[i + 1]);
        }
        GGML_ASSERT((int) eval_pairs.size() == n_outputs);
        compute_logprobs(batch_logits.data(), n_vocab, workers, eval_pairs, eval_results);
        for (float logprob : eval_results) {
            const double v = -double(logprob);
            nll += v;
            nll2 += v * v;
            count++;
        }
        ...
        LOG("[%zu] answer_tokens=%zu ppl=%.4lf,", is + 1, answer_tokens.size(), std::exp(mean));
    }
    LOG("\n");

    if (count < 2) {
        LOG_ERR("%s: not enough answer tokens\n", __func__);
        return;
    }
    nll2 /= double(count);
    nll /= double(count);
    const double ppl = std::exp(nll);
    double var = nll2 - nll * nll;
    if (var > 0) {
        const double se = std::sqrt(var / double(count - 1));
        LOG_INF("Final estimate: answer-only PPL = %.4lf +/- %.5lf over %ld answer tokens from %ld samples\n",
                ppl, se * ppl, (long) count, (long) used_samples);
    } else {
        LOG_INF("Final estimate: answer-only PPL = %.4lf over %ld answer tokens from %ld samples\n",
                ppl, (long) count, (long) used_samples);
    }
}
...
int main(int argc, char ** argv) {
    ...
    const bool ppl = !params.hellaswag && !params.winogrande && !params.multiple_choice && !params.kl_divergence && !params.answer_ppl;
    ...
    if (ppl) {
        ...
        if (params.kl_divergence || params.answer_ppl) {
            params.n_parallel = 1;
        } else {
    ...
    } else if (params.answer_ppl) {
        answer_ppl_score(ctx, params);
